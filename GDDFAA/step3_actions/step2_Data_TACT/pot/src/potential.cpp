...
double Potential_JS::DeltaGuess(const VecDoub& x){
	...
	// gjy note: Guess not Gauss; 当前点的局部的?
	double R = ::norm<double>({x[0],x[1]});
	//gjy note: norm<double>({x[0],x[1]}) is conflix with std::norm(const std::complex<_Tp>&), so add :: or set namespace
	//on error befoce because no include<Eigen> and std::norm()
	// error:
	// src/potential.cpp:153:37: error: call of overloaded ‘norm<double>(<brace-enclosed initializer list>)’ is ambiguous
	//   double R = norm<double>({x[0],x[1]});
	//                                      ^
	// In file included from ../general/coordtransforms/inc/coordsys.h:36:0,
	//                  from src/potential.cpp:37:
	// ../general/utils.h:104:3: note: candidate: c norm(const std::vector<c>&) [with c = double]
	//  c norm(const std::vector<c> &a){
	//    ^~~~
	// In file included from /usr/include/Eigen/Core:80:0,
	//                  from /usr/include/Eigen/Dense:1,
	//                  from /usr/include/Eigen/Eigen:1,
	//                  from /home/darkgaia/0prog/Stackel/tact-master/pot/inc/data_interface.h:17,
	//                  from inc/potential.h:73,
	//                  from src/potential.cpp:39:
	// /usr/include/c++/7/complex:663:5: note: candidate: _Tp std::norm(const std::complex<_Tp>&) [with _Tp = double]
	//      norm(const complex<_Tp>& __z)
	//      ^~~~
	// src/potential.cpp: In member function ‘virtual double GalPot::Phi(const VecDoub&)’:
	// src/potential.cpp:615:37: error: call of overloaded ‘norm<double>(<brace-enclosed initializer list>)’ is ambiguous
	//   double R = norm<double>({x[0],x[1]});
