...
////gjy add
// int derivs(double t, double t_tot, const double y[], double f[],void *params){ 
// 	/* derivatives for orbit integration */
// 	Potential_JS *P = (Potential_JS*) params;
// 	VecDoub x = {y[0],y[1],y[2]};
// 	VecDoub F = P->Forces(x, t_tot);
// 	for(int i=0;i<3;i++){
// 		f[i]=y[i+3]; f[i+3]=F[i];
// 	}
// 	return GSL_SUCCESS;
...
// std::cout<<"Orbit::integrate() is called\n"; //gjy note
	...
	while((t_interval-start)>0.){
		...
		step=step_size*tnow/tnorm; //gjy note: ??
		O->step(start,start+step,y,step); //gjy note: ??
		for(int i=0;i<6;i++) xf[i]=y[i];
		results_.push_back(xf);
		start+=step;
// DEBUG_PRINT_V1d(1, (VecDoub){start, step}, "oiss");
// DEBUG_PRINT_V1d(1, xf, "oixv");
	}
// DEBUG_PRINT_V1d(1, xf, "oixv_out");
	return xf;
}

////gjy add:
// integ by SCF, use funcs in DI, static potential??
// T_orb estimated by point mass??
// VecDoub Orbit::integrate(const VecDoub& x, const double& t_tot, double t_interval, double step_size, bool adaptive, bool is_use_data_potential){
// 	/* Integrate from time t=0 to t=t_interval at regular steps step_size */
// 	/* and store results in the array results							  */
// 	double y[6]; VecDoub xf(6);
...
// 	double tnow = 1., step = step_size, tnorm=1.;
// 	if(adaptive)
// 		tnorm=Pot->torb(x,t_tot)/10.; //+
// 	while((t_interval-start)>0.){
// 		if(adaptive) tnow = dynamic_time(results_.back());
// 		step=step_size*tnow/tnorm;
// 		O->step(start,start+step,y,step); 
// 		/* gjy notes: orbit O.integrate输入里就全部结果了; 初始化时在ode O里联系了引力势Potential_JS; */
// 		/* 在ode O.step()进行运算, 其定义在GSLInterface.h); 其第一个参数就是derivs()函数, 里面就有某点??的引力势, 所以改后在derivs()里改引力势pot; 经过计算后的这个y就是轨道下一步结果 */
// 		for(int i=0;i<6;i++) xf[i]=y[i];
// 		results_.push_back(xf);
// 		start+=step;
// 	}
