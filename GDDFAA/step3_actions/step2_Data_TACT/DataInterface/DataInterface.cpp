...
int read_params(string path_IC_1)
{
    ...
    for(int i=1;i<100+1;i++){ //max line in file??
        // int l = fscanf(fp, "%s", s0);
        // std::cout<<s0[0]<<std::endl;
        // printf("read line %d:\n", i);
        fscanf(fp, "%s %s", s0, s);
        if(s0[0]=='#' || s0[0]=='%' || s0[0]=='!' || (s0[0]=='/' && s0[1]=='/')){
            fscanf(fp, "%[^\n]%*c", leftLine);
            // char c = '\0';
    ...
    std::cout<<"count of particles: "<<N_allPtcs<<std::endl;
...
int Snapshot::dispersion_discrete_all(int coor, int nn){
        ...
        for(int id=0+1;id<NumPart+1;id++){
			...
			std::array<double, Dimension> x_tgt_arr = {P[id].Pos[0], P[id].Pos[1], P[id].Pos[2]};
			vector<int> nearest_idx = pkdtree->find_k_nearest<Distance::euclidean>(nn, x_tgt_arr);
			// vector<int> nearest_min = pkdtree->find_k_nearest<Distance::euclidean>(1, x_tgt_arr);
...
vector<double> Snapshot::dispersion_interp(const vector<double>& x, int nn){
		...
		std::array<double, Dimension> x_tgt_arr = {x[0], x[1], x[2]};
		vector<int> nearest_idx = pkdtree->find_k_nearest<Distance::euclidean>(nn, x_tgt_arr);
...
int Snapshot::process_tosecondhand(const double* const QP, int WhereWrite, int WhatCannonical, double* h_mat, int nknn){
		...
		std::array<double, Dimension> Q_target_arr = {QP[0], QP[1], QP[2]};
		std::array<double, Dimension> P_target_arr = {QP[3], QP[4], QP[5]};
		std::array<double, Dimension*2> QP_target_arr = {QP[0], QP[1], QP[2], QP[3], QP[4], QP[5]};
        vector<int> nearest_idx;
        if(WhatCannonical==0){ //tree for x: 3*N
            // this->loadtree(pk, 0); //??
            nearest_idx = this->pkdtree->find_k_nearest<Distance::euclidean>(nknn, Q_target_arr);
        }else if(WhatCannonical==1){ //tree for v: 3*N
            nearest_idx = this->pkdtree1->find_k_nearest<Distance::euclidean>(nknn, P_target_arr);
        }else if(WhatCannonical==2){ //tree for x rho: 3*N
		    nearest_idx = this->pkdtree->find_k_nearest<Distance::euclidean>(nknn, Q_target_arr);
        }else if(WhatCannonical==3){ //tree for xv: 6*N
		    nearest_idx = this->pkdtree2->find_k_nearest<Distance::euclidean>(nknn, QP_target_arr);
        }
        ...
        if(WhatCannonical==5){ //DF_J
            ...
            DF /= N_allPtcs; //number -> frequency -> probability
            for(int ix=0;ix<Dimension;ix++){
                barP[ix] /= nknn;
                barP2[ix] /= nknn;
                sigP[ix] = sqrt( barP2[ix]-barP[ix]*barP[ix] );
