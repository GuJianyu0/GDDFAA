...
public:
    ...
    void load_orbit_data(bool is_center=true); //centeralized in this function
    vector<double> get_orbit_info_by_idx(int idx);

    double foci_by_shape(int swit);

    int main_1_b2(double ef[N_var]);
    int main_2_a2(double ef[N_var]);
    int main_3_xyz_to_lmn();
};
...
template<int dim>
void SCFAEF<dim>::load_orbit_data(bool is_center)
{
    FILE* fp = nullptr;
    printf("Read orbit data ...\n");
    if( !(fp=fopen("Orbit.dat","r")) ){
        printf("Can not open Orbit.dat!\n");
        exit(1);
    }
...
template<int dim>
int SCFAEF<dim>::main_1_b2(double ef[N_var])
{
    FILE* finp;
    FILE* fout;
    int i;
...
template<int dim>
int SCFAEF<dim>::main_2_a2(double ef[N_var])
{
    FILE* finp;
    FILE* fout;
    int i;
...
template<int dim>
int SCFAEF<dim>::main_3_xyz_to_lmn()
{
    FILE* finp;
    FILE* fout;
    int i;
