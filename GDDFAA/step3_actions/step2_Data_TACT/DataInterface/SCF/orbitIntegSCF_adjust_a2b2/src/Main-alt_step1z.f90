...
!###############################################################
!###############################################################
subroutine main_for_orbit_integrating (dist)
  implicit none
...
! 确定大致的搜索范围
  ...
  call get_pot(x,pot)
  vmod = sqrt(abs(pot)) !gjy comment: sqrt(abs(E-pot)) ??
  print*,"y =",x(2), "v_circ=",vmod
  vstep = vmod / 20.0
  v1 = vmod
  ! 从y_i 出发，积分半个轨道后与Y轴相交于 y_f (<0)
...
subroutine compute_dy(x,v,set_v,n_call,dy)
  ...
  ! 以防万一
  dy = abs(dy)
  write(*,"('V_z = ',f9.3, ' |Delta y|= ', f14.8, 2x,'time = ', f11.3)") set_v, dy, t1-t0
...
!			/* parameter define */
   ...
   logical,parameter:: exact_on_plane = .TRUE.
...
!	/* PREDICTION for PARTICLE */
  ...
  ! print*,"get_force=",ap !gjy add: NaN NaN NaN
  if( exact_on_plane) then
    ap(1)=0.0
    apdot(1)=0.0
  end if
  ...
  if( ecode == 99 ) then
    print*,"ecode=",ecode !gjy add: NaN NaN NaN
...
! 输出监控信息
  if( t_global >= t_info ) then	!/* useful info output */
    ...
    ! print*,"t_global=",t_global !gjy add
    ! print*,"t_info=",t_info !gjy add
...
! 输出轨道信息
  if( t_global >= t_save .and. outflag==1 ) then
    call WriteToFile( t_global, x, v, n_step )
...
! 绕x轴旋转的轨道，在YZ平面，记录多个整周期中在y轴距离的最大、最小值
! 在搜索vz的过程中，不输出到文件
  if( x(3)*xold(3)<0.0 .and. x(2)>0.0 .and. outflag==0 ) then ! 穿越Y轴
    yf = 0.5*(x(2)+xold(2))
    if( yf < ymin) ymin=yf
    if( yf > ymax) ymax=yf
  end if
