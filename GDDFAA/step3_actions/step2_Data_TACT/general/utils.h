...
// #include "/home/darkgaia/0prog/Stackel/tact-master/general/GSLInterface/GSLInterface.h"

// ============================================================================
// gjy note: tact作者写的很实用的计算工具小文件

typedef std::vector<int> VecInt;
typedef std::vector<double> VecDoub;
typedef std::vector<std::vector<double> > MatDoub; //like vec cross-product vec
typedef std::vector<std::vector<std::vector<double> > >MatMatDoub;

const double PI = 3.14159265359;
const double TPI = 6.283185307; //twoPI
...
inline double GaussLegendreQuad(double (*func)(double, void*),double a,double b, void *p = NULL, int order = 10){
    ...
    // gjy note: params (被积函数(被积自变量, 被积函数的参数??), 积分下限, 积分上限, 传进去的被积函数的参数, 阶数(??))
...
// Calculate the determinant recursively. //递归计算行列式
template<class c>
