...
// std::cout<<"rkck 1: dydx[0] = "<<dydx[0]<<", "; //gjy add
	for (i=0;i<N;i++)
...
// std::cout<<"rkck yerr = "<<yerr[0]<<": dydx[0]="<<dydx[0]<<", h="<<h<<", dc1="<<dc1<<", ak3[0]="<<ak3[0]<<"\n"; //gjy add
}
		  ...
		  void (*derivs)(double, double*, double*,void*), void *params)
{
	...
	h=htry;
	for (;;) {
// std::cout<<"rkqs() h1 = "<<h<<", "; //gjy add
		rkck(y,dydx,N,*x,h,ytemp,yerr,derivs,params);
		errmax=0.0;
		for (i=0;i<N;i++) errmax=MAX(errmax,fabs(yerr[i]/yscal[i]));
		errmax /= eps;
// // std::cout<<"rkqs() for: yerr[i] = "<<yerr[i]<<", yscal[i] = "<<yscal[i]<<", errmax = "<<errmax<<", "; {}//gjy add
// std::cout<<"errmax = "<<errmax<<", x2[1]=ytemp[1]="<<ytemp[1]<<", "; //gjy add
		if (errmax <= 1.0) break;
		htemp=SAFETY*h*pow(errmax,PSHRNK);
		h=(h >= 0.0 ? MAX(htemp,0.1*h) : MIN(htemp,0.1*h));
// std::cout<<"h2 = "<<h<<", htemp = "<<htemp<<"; "; //gjy add
		xnew=(*x)+h;
		if (xnew == *x) //gjy note: 此时h->0, 步子不再增长
		{
			cerr << "At x=" << xnew;
