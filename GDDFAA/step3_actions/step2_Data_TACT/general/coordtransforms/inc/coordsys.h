...
class ConfocalEllipsoidalCoordSys{
	...
	public:
		...
		*/
		...
		inline double alpha(void){ return Alpha;} //只是因为\Alpha是私有的变量, 所以多此一举弄个直接返回的函数
		...
		*/
		...
		VecDoub tau2PSQ(const VecDoub& tau);
		/* Calculates (lambda,...; P^2 l_dot^2,...)(tau) given 6D x vector */ //gjy add
		/*!
			\param  3D
			\return 6D
		*/
		VecDoub xv2tp(const VecDoub& x);
		/* Calculates (lambda,...; P^2 l_dot^2,...)(tau) given 6D x vector */ //gjy add
		/*!
			\param  6D
			\return 6D
		*/
		VecDoub tau2tp(const VecDoub& tau);
		/* Calculates (lambda,...; P^2 l_dot^2,...)(tau) given 6D tau vector */ //gjy add
		/*!
			\param 6D
			\return 6D
		*/

		// VecDoub xv2tp_other(const VecDoub& x);
		// /* Calculates (lambda,...; P^2 l_dot^2,...)(tau) given 6D tau vector */ //gjy add
		// /*!
		// 	\param 6D
		// 	\return 6D
		// */
		// VecDoub xv2tp_GF(const VecDoub& x);
		// /* Calculates (lambda,...; P^2 l_dot^2,...)(tau) given 6D tau vector */ //gjy add
		// /*!
		// 	\param 6D
		// 	\return 6D
