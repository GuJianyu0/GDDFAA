...
/* gjy note: 这个是作者JS写的用到的函数集成, 也用于容易调用 */
...
class root_find{
    ...
    public:
    	root_find(double tol,int max_iter)
    		...
    		iter=0;eps=1.e-8;
       		T = gsl_root_fsolver_brent; //gjy note: code that might have not-finite-value bug when use data potential
    	...
    	void bracket(){
			...
			if(xhi<xlo){double xt = xlo; xlo=xhi; xhi=xt;} //gjy note: 大小相反就交换
			double diff = (xhi-xlo)/2.;
			while(GSL_FN_EVAL(&F,xlo)*GSL_FN_EVAL(&F,xhi)>0.){ //gjy note: 同号就扩大范围
				xlo-=diff;xhi+=diff;
			}
		}

     	double findroot(double(*func)(double,void *),double xlo1,double xhi1, void *p=NULL){
			//gjy note: 参数: 目标函数=0, *func的第一个double参数关于 区间low到high的根, 目标函数的参数
     		// finds root in interval [xlo1,xhi1]
    		F.function = func; F.params=p; xhi = xhi1; xlo = xlo1;
			// std::cout<<GSL_FN_EVAL(&F,xlo)<<" "<<GSL_FN_EVAL(&F,xhi)<<std::endl; //gjy add
// std::cout<<"Here_211"; //gjy add
     	    if(GSL_FN_EVAL(&F,xlo)*GSL_FN_EVAL(&F,xhi)>0.){
     			bracket();
     		}
// std::cout<<"Here_212"; //gjy add
       		gsl_root_fsolver_set (s, &F, xlo, xhi); //gjy note: code that might have not-finite-value bug when use data potential
// std::cout<<"Here_213"; //gjy add
			do{ //gjy note: Brent??
// std::cout<<"Here_2131"; //gjy add
				iter++;
				status = gsl_root_fsolver_iterate (s);
// std::cout<<"Here_2132"; //gjy add
				root = gsl_root_fsolver_root (s);
// std::cout<<"Here_2133"; //gjy add
				xlo = gsl_root_fsolver_x_lower (s);
// std::cout<<"Here_2134"; //gjy add
				xhi = gsl_root_fsolver_x_upper (s);
// std::cout<<"Here_2135"; //gjy add
				status = gsl_root_test_interval (xlo, xhi, tol, eps);
// std::cout<<"GSLInterface.h: findroot(): iter = "<<iter<<"\n"; //gjy add
	...
	public:
		...
		void step(double tstart, double tfinish, double *y, double step){
			...
			while ((tfinish-tstart)*direction>0){
				// std::cout<<"GSLInterface.h: "<<tstart<<" "<<tfinish<<" "<<direction<<"\n"; //gjy add //tstart几乎不增长
      			int status = gsl_odeiv2_evolve_apply(e, c, s, &sys, &tstart, tfinish, &h, y); //gjy note: at /home/darkgaia/0prog/Stackel/gsl-1.16/ode-initval2/evolve.c
      			// int status = gsl_odeiv2_evolve_apply_fixed_step(e, c, s, &sys, &tstart, h, y); //gjy add //cannot use no step upward??
...
class minimiser{
	private:
		...
		gsl_multimin_fminimizer *s; //gjy note: 多维函数最小值 ??
		gsl_vector *ss, *x;
	...
	public:
		minimiser(double(*func)(const gsl_vector *v, void *params),double *parameters,
		 int N, double *sizes, double eps, void *params):N_params(N), eps(eps){
			...
			s = gsl_multimin_fminimizer_alloc (T, N_params); //s赋值
		...
		double minimise(double *results,unsigned int maxiter,bool vocal){
			...
			//& 这里改第一个参数results; 这里第一个参数应当是double*, 应输入相应量首地址, 但这里其实可以操作后面的地址
			return s->fval;
		}
