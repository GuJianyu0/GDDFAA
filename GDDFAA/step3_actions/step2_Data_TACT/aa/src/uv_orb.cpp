...
void uv_orb::fillDeltagrids(const std::string& file){
    ...
    for(int i=0; i<NE;i++){
std::cout<<"here11 NE i = "<<i<<",,,,,,,,,,,,,,,,,,,,,,\n";//gjy add
std::cout<<"here12 E_delta[i] = "<<E_delta[i]<<"\n";//gjy add
        double R = .9*Pot->R_E(E_delta[i]); //gjy note: 暂时 不断E>Phi
std::cout<<"here13 R(E_delta) = "<<R<<"\n";//gjy add
        VecDoub DeltaResult(2,0.);
        for(double j=0;j<NL;j++){
            L_delta[i][j] = Pot->L_circ(R)*(j*0.8/(double)(NL-1)+0.001); //??forces不明
std::cout<<"here14 NL j = "<<j<<",,,,,,,,,,,,,,,,,,,,,,\nL_delta[i][j]="<<L_delta[i][j]<<", find_best_delta start!!\n";//gjy add //??以下NL很慢, 而且老是回去
            find_best_delta DD(Pot, E_delta[i], L_delta[i][j]);
            DeltaResult = DD.delta(R); //gjy note: 此find_best_delta DD.delta() 有时报错: (At x=0) stepsize underflow in rkqs
			//at XX/Stackel/tact-master/aa/src/get_closed_Rz.cpp 
			//at XX/Stackel/tact-master/general/jamestools/numrec/pressstuff.cpp
std::cout<<"here15\n";//gjy add
