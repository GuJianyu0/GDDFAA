...
static double *dmatrix_SF(int n){
    // creates array of length n and fills with zeroes
    //与VecDoub不同是, 这是一个函数, 创建返回一个数组(首地址)洄的n长度(全0)
...
double Stackel_Fitted_Potential::chi(double lambda, double nu){
    // chi(lam, nu), chi所用的phi就是--
    // 解析的 StackelOblate_PerfectEllipsoid::Phi_tau(const VecDoub& tau) 于potential.cpp
...
double Stackel_Fitted_Potential::flam(double lambda){
    ...
    return ((GaussLegendreQuad(&chiNu,limits[2],limits[3],&SFP,50)/N)-0.5*chibar); //Zeeuw15
}
...
double Stackel_Fitted_Potential::find_turning(VecDoub x, VecDoub y, double step, int lamnu){
    ...
    return tauupper[index-3]; //出?有根时所在\tau
}
...
void Stackel_Fitted_Potential::find_limits(VecDoub x){
    ...
    newalpha(-TruePot->DeltaGuess(y)+gamma()); // Initial guess of alpha; \alpha===-a^2
    ...
    while (alldone1*alldone2*alldone3*alldone4==0){
        ...
        ynew = Orb->integrate(y,step,step); //下一(轨道积分数值轨迹)步的y(用y是因为想复用但直接引用怕变?)
        // Refine our guess of alpha
        if(steps%5==0 && steps<=STEPMAX){ //每5个点取一个点算mewalpha
            ...
            newalpha(((n-1)*alpha()+new_alpha)/(double)n); //其实在做平均
            n++;
        }
        taunew = xv2tau(ynew);
        ...
        if(taunew[3]*tau[3]<0.0
           && lamdotdot>0.0
           && steps>STEPMAX //步骤应该都到了?, 轨道积分用全了
           && alldone1==0){
            limits[0] = find_turning(y,ynew,step,0);alldone1=1; //lamda_minus
        }
...
void Stackel_Fitted_Potential::fit_potential(VecDoub x){
    ...
    if(fabs(limits[2]-limits[3])<SMALL) limits[3]+=SMALL; //作用 让lim2-lim3>sml
    ...
    lambdagrid=dmatrix_SF(DATAPOINTS); //到这里只是创建
    ...
    for(int i=0;i<DATAPOINTS;i++){
        ...
        flamgrid[i] = flam(lambdagrid[i]); //自变量?lamda是线性插值, 其势函数flamdagrid(lamda)按数值法接着
        fnugrid[i] = fnu(nugrid[i]);
    }

    // Create splines for interpolation
    y2Lamgrid = dmatrix_SF(DATAPOINTS); //Stores second derivs for best-fit of flam(grid)
    y2Nugrid = dmatrix_SF(DATAPOINTS);
    spline(lambdagrid, flamgrid, DATAPOINTS, 1e30, 1e30, y2Lamgrid); //三次样条插值, 传入参数都是引用(地址)
...
VecDoub Actions_StackelFit::actions(const VecDoub& x, void*params){
    ...
    SFP->fit_potential(x);
    Actions_AxisymmetricStackel AS(SFP); //AS是模型轴称势的实体, AFP是对拟合的势求aa的
    return AS.actions(x); //给相应的势传入参数, 直接调用; AS.actions()里的StackelOblate_PerfectEllipsoid::x2ints()的参数x是任意的
    //理论上的actions不随时间变化, 但程序里actions不论模型还是fudge, 都是x的函数, 所以出现J16图2的actions随时间图
    //下面注释实例(以及程序), 用轨道积分给出xv点, 然后在给定点按照模型算actions, 每点的actions参数都不一样?
}
