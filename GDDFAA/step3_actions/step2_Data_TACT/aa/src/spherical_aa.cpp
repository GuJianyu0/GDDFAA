...
static double p_r(double r, void *params){ //gjy note: 直接用的球称势的求r动量的公式
...
VecDoub Actions_Spherical::actions(const VecDoub &x, void *params){
    ...
    double E = Pot->H(x), L = Pot->L(x), Lz = Pot->Lz(x); //gjy note: 在Pot_JS里都定义函数了
    if(E>0.){
        return {std::numeric_limits<double>::infinity(),Lz,L-fabs(Lz)}; //E>0, inf
    }
    VecDoub limits;
    if(sqrt(x[3]*x[3]+x[4]*x[4]+x[5]*x[5])==0.) limits={0.,r};
    else limits = find_limits(r,E,L); //gjy note: length is 2
    double taubar = 0.5*(limits[0]+limits[1]); //gjy note: ??????
    double Delta = 0.5*(limits[1]-limits[0]);
    Actions_Spherical_data_struct Act(Pot,E,L,Lz,Delta,taubar);
    double JR = Delta*GaussLegendreQuad(&J_r,-PI/2.,PI/2.,&Act)/PI; //gjy note: 一种积分; ??
...
VecDoub Actions_Spherical::angles_and_freqs(const VecDoub &x){
    ...
    double ThetaR = dSRdH*OmegaR; //gjy note: ????????????????
...
VecDoub Actions_Spherical::Hessian(const VecDoub &x){ //gjy note: Hessian Matrix?
...
//-----------------------------------------------------------------------------------------------
//gjy add
static double p_r1(double r, void *params){ //gjy note: 直接用的球称势的求r动量的公式
    // need to set taubargl and Deltagl
    Actions_Spherical_DataPotential_limits_struct * AS = (Actions_Spherical_DataPotential_limits_struct *) params;
    return 2.*AS->E-2.*AS->Pot->Phi_r(r)-AS->L*AS->L/r/r;
}

static double J_r1(double theta,void *params){ //gjy note: theta是个形参, 关于积分中自变的被积函数??
    // need to set taubargl and Deltagl
    Actions_Spherical_DataPotential_data_struct * AS = (Actions_Spherical_DataPotential_data_struct *) params;
    double r = sin(theta)*AS->Delta+AS->taubar; //gjy note: ??????????????????????
    return cos(theta)*sqrt(MAX(0.,2.*AS->E-2.*AS->Pot->Phi_r(r)-AS->L*AS->L/r/r));
}

static double dJrdH1(double theta,void *params){
    // need to set taubargl and Deltagl
    //gjy note: 在当前点的偏微导数值, 并非表达式
    Actions_Spherical_DataPotential_data_struct * AS = (Actions_Spherical_DataPotential_data_struct *) params;
    double r = sin(theta)*AS->Delta+AS->taubar;
...
static double dJrdL1(double theta,void *params){
    // need to set taubargl and Deltagl
    Actions_Spherical_DataPotential_data_struct * AS = (Actions_Spherical_DataPotential_data_struct *) params;
    double r = sin(theta)*AS->Delta+AS->taubar;
    return -cos(theta)*AS->L/r/r/sqrt(MAX(TINY,2.*AS->E-2.*AS->Pot->Phi_r(r)-AS->L*AS->L/r/r));
...
static double dLdL1(double theta,void *params){
    // need to set taubargl and Deltagl
    Actions_Spherical_DataPotential_data_struct * AS = (Actions_Spherical_DataPotential_data_struct *) params;
    double st = sin(theta);
    return AS->L/sqrt(MAX(TINY,AS->L*AS->L-AS->Lz*AS->Lz/st/st));
...
static double d2JdH21(double theta,void *params){
    // need to set taubargl and Deltagl
    Actions_Spherical_DataPotential_data_struct * AS = (Actions_Spherical_DataPotential_data_struct *) params;
    double r = sin(theta)*AS->Delta+AS->taubar;
    double L2r2 = AS->L*AS->L/r/r;
...
static double d2JdHdL1(double theta,void *params){
    // need to set taubargl and Deltagl
    Actions_Spherical_DataPotential_data_struct * AS = (Actions_Spherical_DataPotential_data_struct *) params;
    double r = sin(theta)*AS->Delta+AS->taubar;
    double L2r2 = AS->L*AS->L/r/r;
...
VecDoub Actions_Spherical_DataPotential::find_limits(double r, double E, double L){
    VecDoub limits;
    Actions_Spherical_DataPotential_limits_struct Act(Pot,E,L);
    double r_in=r, r_out=r;
    root_find RF(SMALL,100);
    if(p_r1(r,&Act)>=0.){ //gjy note: ???????
        if(p_r1(0.,&Act)>0) r_in=0.;
        else while(p_r1(r_in,&Act)>=0.0) r_in*=0.9;
        while(p_r1(r_out,&Act)>=0.0) r_out*=1.1;
        limits.push_back(RF.findroot(&p_r1,r_in,r,&Act));
        limits.push_back(RF.findroot(&p_r1,r,r_out,&Act));
    }
    else{limits.push_back(r-TINY);limits.push_back(r+TINY);}
    return limits;
}

VecDoub Actions_Spherical_DataPotential::actions(const VecDoub &x, void *params){
    double r = sqrt(x[0]*x[0]+x[1]*x[1]+x[2]*x[2]);
    double E = Pot->H(x), L = Pot->L(x), Lz = Pot->Lz(x); //gjy note: 在Pot_JS里都定义函数了
    if(E>0.){
        return {std::numeric_limits<double>::infinity(),Lz,L-fabs(Lz)}; //E>0, inf
    ...
    double taubar = 0.5*(limits[0]+limits[1]); //gjy note: ??????
    double Delta = 0.5*(limits[1]-limits[0]);
    Actions_Spherical_DataPotential_data_struct Act(Pot,E,L,Lz,Delta,taubar);
    double JR = Delta*GaussLegendreQuad(&J_r1,-PI/2.,PI/2.,&Act)/PI; //gjy note: 一种积分; ??
    return {JR,Lz,L-fabs(Lz)};
}

VecDoub Actions_Spherical_DataPotential::angles_and_freqs(const VecDoub &x){
    // call actions before
    double E = Pot->H(x), L = Pot->L(x), Lz = Pot->Lz(x);
    if(E>0.){
        return {std::numeric_limits<double>::infinity(),
    ...
    double OmegaR = Delta*GaussLegendreQuad(&dJrdH1,-PI/2.,PI/2.,&Act)/PI;
    OmegaR = 1./OmegaR;
    double Omegap = Delta*GaussLegendreQuad(&dJrdL1,-PI/2.,PI/2.,&Act)/PI;
    Omegap*=-OmegaR;

    VecDoub SPol = conv::CartesianToSphericalPolar(x);

    double thetaLim = asin(MAX(-1.,MIN(1.,(SPol[0]-taubar)/Delta)));
    double dSRdH=sign(SPol[3])*Delta*GaussLegendreQuad(&dJrdH1,-PI/2.,thetaLim,&Act);
    double dSRdL=sign(SPol[3])*Delta*GaussLegendreQuad(&dJrdL1,-PI/2.,thetaLim,&Act);

    double ThetaR = dSRdH*OmegaR; //gjy note: ????????????????

    double dStdL=sign(SPol[5])*GaussLegendreQuad(&dLdL1,PI/2.,SPol[2],&Act);
    // printVector(x);
    double Thetap=dSRdL+dStdL+dSRdH*Omegap;
    if(SPol[5]>0.) Thetap+=PI;
    double LR=Lz/L;
...
double Actions_Spherical_DataPotential::dr0dH(double r, double L){
    return 1./(Pot->dPhi_r(r)-L*L/r/r/r);
}
double Actions_Spherical_DataPotential::dr0dL(double r, double L){
    return -L/r/r/(Pot->dPhi_r(r)-L*L/r/r/r);
}

VecDoub Actions_Spherical_DataPotential::Hessian(const VecDoub &x){ //gjy note: Hessian Matrix?
    std::cerr<<"Currently not working!!"<<std::endl;
    VecDoub Freq(2,0), AF = angles_and_freqs(x);
    Freq[0]=AF[3];Freq[1]=AF[4];
    double E = Pot->H(x), L = Pot->L(x), Lz = Pot->Lz(x);
    ...
    double dDelta=0.5*(dradH-drpdH);
    double dtaubar=0.5*(dradH+drpdH);
    Actions_Spherical_DataPotential_data_struct Act(Pot,E,L,Lz,Delta,taubar,dDelta,dtaubar);
    double d2JdH = Delta*GaussLegendreQuad(&d2JdH21,-PI/2.,PI/2.,&Act)/PI;
    double drpdL = dr0dL(limits[0],L);
    double dradL = dr0dL(limits[1],L);
    dDelta=0.5*(dradL-drpdL);
    dtaubar=0.5*(dradL+drpdL);
    // double d2JdHdL2 = Delta*GaussLegendreQuad(&d2JdHdL1,-PI/2.,PI/2.,&Act)/PI;
    // d2JdH+=Delta*(dr0dH(limits[1])*dJrdH1(PI/2.,&Act)-dr0dH(limits[0])*dJrdH1(-PI/2.,&Act))/PI;
    double dOmegadJR = -Freq[0]*Freq[0]*Freq[0]*d2JdH;
    return {dOmegadJR};
}

void simple_isochrone_orbit_for_movie1(void){
    IsochronePotential Iso(1000000.,3.64);
    Actions_Spherical_DataPotential AS(&Iso);
    VecDoub X ={2.67519,1.04903,-3.08583,175.546,-216.936,63.3228};
    Orbit orbit(&Iso);
    VecDoub QQ=X;double PPtmp = 0., fac = 0.;
    for(int i=0;i<250;i++){
