...
static double min_distance(double y, void *params){ //被findclosed()调用
...
DEBUG_PRINT_I(131);
	// std::cout<<"测试轨道前几个点: "<<std::endl; //gjy add
	printVector(orbit.results()[0]);
	// std::cout<<"结束. "<<std::endl; //gjy add
...
static double min_action(double b, void *params){ //gjy note: called by findalpha()
	...
	for(auto Y: RS->orbit->results()){
		VecDoub i = RS->ATSF->actions(Y); //这个i只是J
...
static double min_lambda(double b, void *params){ //gjy note: called by find_beta()
	root_struct_action *RS = (root_struct_action *) params;
	if(RS->swit==0)RS->ATSF->CS->newalpha(b);
	else if(RS->swit==1)RS->ATSF->CS->newbeta(b); //所以swit是关于alpha还是beta的指标?
	double L = (double)RS->orbit->results().size();
	double means=0.,vars=0.;int index = 0;
	for(auto Y: RS->orbit->results()){
		VecDoub i = RS->ATSF->CS->xv2tau(Y); //相当于在新使beta=b的情况下的轨道点的椭球坐标
		//index=0的i是lambda还是?
		i[index]+=RS->ATSF->CS->alpha(); //改变beta不影响alpha; 所以这个是alpha之偏离?
		if(i[index]==i[index]){means+=i[index];	vars+=i[index]*i[index];} //if没用?
	}
	double f = sqrt(MAX(0.,vars/L-means*means/L/L)); //标准差 属的 \tau_Y[index?]
	return f;
}
...
double lmn_orb::find_beta(double E, double a_init, double b_init){ //gjy note: called by fillGrads(), to find beta
	if(E<E0){
...
DEBUG_PRINT_I(1003);
	...
	} // or minimize actions, 用到最小化J方法了
	else{
		double P = Pot->Phi({0.,y,0.});
		double p = sqrt(2.*(E-P)); //说明这个lmn_orb::_Pot真实势就是那个势作loop的势; 且此y就是每一次的y_I
		// Orbit orbit(Pot);
		SymplecticOrbit orbit(Pot); //gjy changed
		...
		double step = torb*10./maxsteps;
		VecDoub QQ=orbit.integrate(X, maxsteps*step, step);
		Actions_TriaxialStackel_Fudge ATSF(Pot, a_init, b_init); //这里果然fudge了, 但这个函数里后面不用来算actions而是为调用alpha()
		root_struct_action RS(&ATSF,&orbit,1);
		double a = -1.-y*y;//
		// a = ATSF.CS->alpha()+0.0001; //为使之, 在z=0面
		double g = ATSF.CS->gamma()-1e-6;

		auto min_func = /*&min_action;//*/&min_lambda;

		double down = min_func(a,&RS), up = min_func(g,&RS); //down是在这个beta(swit==1)下lambda的标准差?
		double av = (a+g)/2.;
		...
		while(avM>down){ //标准差超出极端时的(beta=alpha了)时, 贴过去直到让Delta小于down的; 什么意义, 难道只为了下面if==up?
		...
		if(avM==up) return -1.001;
			...
			int status = 0;
			minimiser1D min(min_func,av,a,g,1e-3,0.,&status,&RS); //最后还是要用最小函数minimize找到最小的焦距min_func
...
double lmn_orb::find_alpha(double E, double alpha_i, double beta){ //gjy note: called by fillGrads()
...
VecDoub lmn_orb::find_ab_from_box(double E){ //gjy note: not called by functions in this .cpp file
...
VecDoub lmn_orb::find_best_alphabeta(const VecDoub& X){ //用了J5的最小化法, 但没被调用??
	double torb = Pot->torb(X);
...
void lmn_orb::readDeltagrids(const std::string& file){
	...
	while(infile>>tmp>>tmp2>>tmp3>>tmp9>>tmp9>>tmp9>>tmp9>>tmp9){ //gjy changed
		...
		if(idx==0){E0 = tmp;} //gjy add: reset {Emin, Emax} to avoid bad outter interpolation
		idx++;
	}
	Emax = tmp; //gjy add: reset {Emin, Emax}
	infile.close();
}
...
DEBUG_PRINT_I(12051);
	...
	//gjy note: 找Delta(ab)的, 是本文件主要的步骤; 这个函数在类对象初始化时就被调用了
...
VecDoub lmn_orb::findDelta_interp(double En){ //gjy note: this is called
	double a, b;
	// if(En>Emax){
	...
	if(En>Emax){
		...
		int i=0;
		while(E[i]<En and i<NE-1)	i++; //gjy note: 找到对应的E[i]接近En
		a = (Alpha[i]-Alpha[i-1])/(E[i]-E[i-1])*(En-E[i-1])+Alpha[i-1]; //线性, \Delta E=\Delta alpha, 把a(即alpha)插值到接近的E[i]
...
VecDoub lmn_orb::actions(const VecDoub& x, void* params){
	...
	if(params!=nullptr){
		...
		return ATSF.actions(x); //gjy note: use designated focus
	}
	else{
