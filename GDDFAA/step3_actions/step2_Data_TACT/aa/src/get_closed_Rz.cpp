...
// ============================================================================
/// \brief Finding best coordinate system from shell orbits //gjy note: 文件目的
///
...
double delta_st::find_focus(void){
    ...
    VecDoub data(9,0.);
    MatDoub x(N,VecDoub(3,0.));
    double Rsc=Rp[0];
    for(int k=0;k<N;++k){
        x[k][0]=Rp[k]*Rp[k]/Rsc/Rsc;
        x[k][1]=zp[k]*zp[k]/Rsc/Rsc;
        x[k][2]=1.;
    }
    ...
    gsl_matrix_view m
        = gsl_matrix_view_array (&data[0], 3, 3);
    gsl_vector_complex *eval = gsl_vector_complex_alloc (3);
    gsl_vector_complex *tmp = gsl_vector_complex_alloc (3);
    ...
    VecDoub evec_m(3,0.);
    gsl_matrix_complex_get_col (tmp, evec, 2);
    for (int j = 0; j < 3; ++j)
    ...
    double c=evec_m[1],g=evec_m[2],a=evec_m[0],b;
    double up = -a*c*g;
    double down1=a*a*c;
    double down2=a*c*c;
...
// std::cout<<"derives() *y = "<<y[0]<<" "<<y[1]<<" "<<y[2]<<" "<<t<<", "; //gjy add
// std::cout<<"f = "<<f[0]<<" "<<f[1]<<" "<<f[2]<<" "<<dP[0]<<", "; //gjy add //若换用dPhi/dx而不是插值的F_xyz??
    for(int i=0;i<2;i++){
        dydt[i]=y[i+2];
        dydt[i+2]=-dP[i];
    }
...
static double goround(double x, void *params){//throws up and determines Rdown
    ...
    if(vsq<0){
        printf("goroung(): vsq<0: %f %f\n",x,vsq);
        // exit(0); //gjy add print
    }
    double x2[4]={x,0.,0,sqrt(vsq)};
std::cout<<"goround() x = "<<x<<", vsq = "<<vsq<<", "; //gjy add
...
// std::cout<<"goroung() while()2 x2 _0123 = "<<x2[0]<<" "<<x2[1]<<" "<<x2[2]<<" "<<x2[3]<<", "; //gjy add
// std::cout<<"dxdt _0123 = "<<dxdt[0]<<" "<<dxdt[1]<<" "<<dxdt[2]<<" "<<dxdt[3]<<"\n"; //gjy add
...
double find_best_delta::dPhi_eff(double x){ //gjy note: effective (axisymmetric) potential
...
double find_best_delta::sorted(double x0){
    int k=0;
// std::cout<<"vsq sorted x0 = "<<x0<<", vsqx(x0) = "<<vsqx(x0)<<", dPhi_eff = "<<dPhi_eff(x0)<<"\n"; //gjy add
    while(vsqx(x0)<0){
        if(dPhi_eff(x0)<0) x0*=1.1;
        else x0*=.9;
        k++;
        if(k>50){
            printf("vsq<0 till k=50: %f %f %f\n",x0,E,Lzsq);
            // exit(0);
        }
    }
    return x0;
...
VecDoub find_best_delta::delta(double x0, int iter){
    ...
    // finds closed shell
    double x00=x0;
std::cout<<"find_best_delta::delta() here21 x0 0 = "<<x0<<"\n"; //gjy add
    x0=sorted(x0);// ensure vsq>=0
    double x1,dx0,dx1;
    dx0=goround(x0,this); //??
    double dx=.8; x1=sorted(x0/dx); dx1=goround(x1,this);
std::cout<<"here22 dx0, dx1 = "<<dx0<<" "<<dx1<<"\n"; //gjy add
    int k=0,bo=0;
...
std::cout<<"here23 while() k = "<<k<<", x0 = "<<x0<<"\n";//gjy add
        k++;
        if(k==21){
    ...
    // for(int kk=0;kk<np;++kk)std::cout<<Ri[kk]<<" "<<zi[kk]<<" "<<E<<" "<<Lzsq<<" "<<dtmp<<" "<<dtmp_a<<" "<<sqrt(dtmp2)<<std::endl;
