...
VecDoub Actions_Genfunc::find_box_params(const std::vector<VecDoub> &orbit_samples){
    // std::cout<<"genfunc_aa.h: Actions_Genfunc::find_box_params() is called."<<"\n"; //gjy add
    VecDoub xmax(3,TINY);
...
VecDoub Actions_Genfunc::find_isochrone_params(const std::vector<VecDoub> &orbit_samples){
    ...
    // std::cout<<"genfunc_aa.h: Actions_Genfunc::find_isochrone_params() is called."<<"\n"; //gjy add
...
VecDoub Actions_Genfunc::actions(const VecDoub &x, void *params){
	...
	// std::ofstream outfile1; //gjy add
	// outfile1.open("XX/0prog/Stackel/tact-master/aa/20210309_GF_innerdata.txt");
    // std::cout<<"Actions_Genfunc::actions(): TargetPot->torb(x) -- before is called. "<<"\n"; //gjy add
    double timescale = TargetPot->torb(x);
    // outfile1<<"AGaa1 timescale = "<<timescale<<"\n"; //gjy add
    // std::cout<<"Actions_Genfunc::actions(): TargetPot->torb(x) -- after  is called. "<<"\n"; //gjy add
    Actions_Genfunc_data_structure AAR = 
        Actions_Genfunc_data_structure(timescale*4., 120, 6, 
        // Actions_Genfunc_data_structure(timescale*8., 300, 8, //gjy changed: this line original
        1e-8, 4801, timescale*128.1, symmetry=="axisymmetric"?true:false
    ); //gjy note: stepsize=第一个参数/第二个参数
    ...
    orbit.integrate(x,AA->total_T,AA->stepsize,false); //gjy note: while(timescale*8-0>0) 死循环了
    // outfile1<<"AGaa2 orbit.results()[][] = "<<orbit.results()[1][0]<<"\n"; //gjy add

    // SymplecticOrbit orbit(TargetPot,50.);
    // orbit.integrate(x,AA->total_T,AA->stepsize,false);

    // outfile1<<"AGaa3"<<"\n"; //gjy add
    // outfile1.close(); //gjy add
    // std::cout<<"AGaa2"<<"\n"; //gjy add
    ...
    VecDoub a(N_SIZE*(N_SIZE+1)/2,0),b(N_SIZE,0.); //gjy note: definition of b[]
    ...
    printf("AGaa: need_to_repeat?? longer_int_window=%d, finer_sampling=%d\n", longer_int_window, finer_sampling); //gjy print
    ...
    if(
      ...
      (total_loop==0 and std::any_of(b.begin(),b.begin()+3,[](double i){return i<0;}))
        ...
        or fac_of_two
            ...
            fatal=true;
            need_to_repeat=true;
            printf("AGaa: b_012 = %f %f %f, fac_of_two = %d\n", b[0], b[1], b[2], fac_of_two); // //gjy print: negative??
    }
                ...
                if(loopi[0]*loopi[0]==1 and la_switch==true){
                    ...
                    if(AF)
                        ...
                        Actions_TriaxialStackel_Fudge ActS(TargetPot,
                                                           -r2/10.,-r2/20.); //gjy note: ??: focus1 = r*r/10;
