...
int allocate_component_arrays(galaxy *gal) {
  ...
  if (!(gal->comp_profile_name =
            (char **)malloc(AllVars.MaxCompNumber * sizeof(char *)))) {
  ...
  if (!(gal->comp_scale_dens = calloc(AllVars.MaxCompNumber, sizeof(double)))) {
    ...
    printf("输入错误gal->comp_scale_dens!!\n\n\n"); //gjy add
    return -1;
  }
  ...
  if (!(gal->comp_spherical_hydro_eq =
            calloc(AllVars.MaxCompNumber, sizeof(int)))) {
    fprintf(stderr,
            "[Error] Unable to allocate comp_spherical_hydro_eq array\n");
    return -1;
  }
...
int allocate_galaxy_potential(galaxy *gal) {
  ...
  for (n = 0; n < gal->nlevel; ++n) {
    ...
    for (i = 0; i < 2 * gal->ngrid[n][0]; ++i) {
      if (!(gal->potential[n][i] =
                calloc(2 * gal->ngrid[n][1], sizeof(double *)))) {
        fprintf(stderr, "[Error] Unable to allocate potential grid\n");
        return -1;
      }
      for (j = 0; j < 2 * gal->ngrid[n][1]; ++j) {
        if (!(gal->potential[n][i][j] =
                  calloc(2 * gal->ngrid[n][2], sizeof(double)))) {
          fprintf(stderr, "[Error] Unable to allocate potential grid\n");
          return -1;
        }
      }
  ...
  if (gal->nlevel > 1) {
    ...
    for (n = 0; n < gal->nlevel - 1; ++n) {
      if (!(gal->potential_ext[n] =
                calloc(2 * gal->ngrid[n][0], sizeof(double *)))) {
        fprintf(stderr, "[Error] Unable to allocate potential grid\n");
        return -1;
      }
      for (i = 0; i < 2 * gal->ngrid[n][0]; ++i) {
        if (!(gal->potential_ext[n][i] =
                  calloc(2 * gal->ngrid[n][1], sizeof(double *)))) {
          fprintf(stderr, "[Error] Unable to allocate potential grid\n");
          return -1;
        }
        for (j = 0; j < 2 * gal->ngrid[n][1]; ++j) {
          if (!(gal->potential_ext[n][i][j] =
                    calloc(2 * gal->ngrid[n][2], sizeof(double)))) {
            fprintf(stderr, "[Error] Unable to allocate potential grid\n");
            return -1;
          }
...
int allocate_galaxy_midplane_dens(galaxy *gal) {
  ...
  for (i = 0; i < gal->ngrid_dens[1]; ++i) {
    // y-axis
    if (!(gal->midplane_dens[i] =
              calloc(gal->ngrid_dens[1], sizeof(double *)))) {
...
int allocate_galaxy_jeans_grid(galaxy *gal) {
  ...
  // Allocate the midplane density grid, starting with x-axis
  if (!(gal->vz2_tilted_mixed =
            calloc(gal->ngrid_jeans[0], sizeof(double *)))) {
    fprintf(stderr, "[Error] Unable to create vz2_tilted_mixed x axis.\n");
    return -1;
  }
  if (!(gal->vr2_tilted_mixed =
            calloc(gal->ngrid_jeans[0], sizeof(double *)))) {
...
int reallocate_variable_arrays(galaxy *gal, unsigned long int npart) {
  ...
  // Allocate particle id numbers array
  gal->id =
      (unsigned long int *)realloc(gal->id, npart * sizeof(unsigned long int));
  if (gal->id == NULL) {
    fprintf(stderr, "[Error] Unable to allocate particle ID numbers.\n");
    return -1;
  }
  ...
  if (gal->theta_cyl == NULL) {
    fprintf(stderr,
            "[Error] Unable to allocate particle theta_cyl coordinates.\n");
    return -1;
  }
...
int allocate_component_arrays_stream(stream *st) {
  ...
  if (!(st->comp_start_part =
            calloc(AllVars.MaxCompNumber, sizeof(unsigned long int)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_start_part array\n");
    return -1;
  }
  if (!(st->comp_mcmc_step = calloc(AllVars.MaxCompNumber, sizeof(double)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_mcmc_step array\n");
    return -1;
  }
  ...
  if (!(st->comp_theta_sph = calloc(AllVars.MaxCompNumber, sizeof(double)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_theta_sph array\n");
    return -1;
  }
  ...
  if (!(st->comp_metal_gradient = calloc(AllVars.MaxCompNumber, sizeof(int)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_metal_gradient array\n");
    return -1;
  }
  ...
  if (!(st->comp_opening_angle =
            calloc(AllVars.MaxCompNumber, sizeof(double)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_opening_angle array\n");
    return -1;
  }
  ...
  if (!(st->comp_turb_gradient = calloc(AllVars.MaxCompNumber, sizeof(int)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_turb_gradient array\n");
    return -1;
  }
  if (!(st->comp_turb_sigma = calloc(AllVars.MaxCompNumber, sizeof(double)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_turb_sigma array\n");
    return -1;
  }
  if (!(st->comp_turb_scale_inj =
            calloc(AllVars.MaxCompNumber, sizeof(double)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_turb_scale_inj array\n");
    return -1;
  }
  if (!(st->comp_turb_scale_diss =
            calloc(AllVars.MaxCompNumber, sizeof(double)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_turb_scale_diss array\n");
    return -1;
  }
  if (!(st->comp_turb_nspec = calloc(AllVars.MaxCompNumber, sizeof(double)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_turb_nspec array\n");
    return -1;
  }
  if (!(st->comp_turb_seed = calloc(AllVars.MaxCompNumber, sizeof(long)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_turb_seed array\n");
    return -1;
  }
  if (!(st->comp_dens_fluct = calloc(AllVars.MaxCompNumber, sizeof(int)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_dens_fluct array\n");
    return -1;
  }
  if (!(st->comp_dens_min = calloc(AllVars.MaxCompNumber, sizeof(double)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_dens_min array\n");
    return -1;
  }
  if (!(st->comp_dens_max = calloc(AllVars.MaxCompNumber, sizeof(double)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_dens_max array\n");
    return -1;
  }
  ...
  if (!(st->comp_gamma_poly = calloc(AllVars.MaxCompNumber, sizeof(double)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_gamma_poly array\n");
    return -1;
  }
  if (!(st->comp_accept_min = calloc(AllVars.MaxCompNumber, sizeof(double)))) {
    fprintf(stderr,
            "[Error] Unable to allocate particle comp_accept_min array\n");
    return -1;
  }
...
int allocate_galaxy_gaussian_grid(galaxy *gal) {
  ...
  if (!(gal->gaussian_field =
            calloc(2 * gal->ngrid_gauss[0], sizeof(double *)))) {
    fprintf(stderr, "[Error] Unable to create turbulence x axis\n");
    return -1;
  }
  for (i = 0; i < 2 * gal->ngrid_gauss[1]; ++i) {
    // y-axis
    if (!(gal->gaussian_field[i] =
              calloc(2 * gal->ngrid_gauss[1], sizeof(double *)))) {
      fprintf(stderr, "[Error] Unable to create turbulence y axis\n");
      return -1;
    }
    // z-axis
    for (j = 0; j < 2 * gal->ngrid_gauss[2]; ++j) {
      if (!(gal->gaussian_field[i][j] =
                calloc(2 * gal->ngrid_gauss[2], sizeof(double)))) {
...
int allocate_stream_gaussian_grid(stream *st) {
  ...
  if (!(st->gaussian_field =
            calloc(2 * st->ngrid_gauss[0], sizeof(double *)))) {
    fprintf(stderr, "[Error] Unable to create turbulence x axis\n");
    return -1;
  }
  for (i = 0; i < 2 * st->ngrid_gauss[1]; ++i) {
    // y-axis
    if (!(st->gaussian_field[i] =
              calloc(2 * st->ngrid_gauss[1], sizeof(double *)))) {
      fprintf(stderr, "[Error] Unable to create turbulence y axis\n");
      return -1;
    }
    // z-axis
    for (j = 0; j < 2 * st->ngrid_gauss[2]; ++j) {
      if (!(st->gaussian_field[i][j] =
                calloc(2 * st->ngrid_gauss[2], sizeof(double)))) {
        fprintf(stderr, "[Error] Unable to create turbulence z axis\n");
        return -1;
      }
    }
...
int create_galaxy(galaxy *gal, char *fname, int info) {
  ...
  if (parse_galaxy_file(gal, fname) != 0) {
    fprintf(stderr,
            "[Error] Unable to initialize from the galaxy parameters file\n");
    return -1;
  }
  ...
  AllVars.H = sqrt(pow(AllVars.H0 / 1e3, 2.0) *
                   (AllVars.Omega_m * pow(1 + AllVars.redshift, 3.0) +
                    AllVars.Omega_k * pow(1 + AllVars.redshift, 2.0) +
                    AllVars.Omega_l));
  // Get the Virial velocity from the parser
  // Set the Virial mass
  if ((gal->m200 == 0.) && (gal->v200 > 0.)) { //gjy note: read M200 and v200
  ...
  gal->r200 = (gal->v200) / (10.0 * AllVars.H); //gjy note: this r200 is not r200 by 200*rho_crit
  // Set the critical density
  rho_crit = 3 * pow(AllVars.H, 2) / (8 * pi * G);
  printf("质量质量200: \n\n\n\nm200 = %f,\nv200 = %f,\nr200 = %f;\nAllVars.H = %f,\nG = %f,\nrho_crit = %f.\n", 
  gal->m200, gal->v200, gal->r200, AllVars.H, G, rho_crit); //gjy add //rho_crit \approx 1.398e-08
  ...
  for (n = 1; n < gal->nlevel; n++) {
    ...
    if (gal->boxsize[n] * gal->boxsize_flatx[n] >
        gal->boxsize[n - 1] * gal->boxsize_flatx[n - 1]) {
      gal->boxsize_flatx[n] = 0.95 * gal->boxsize[n - 1] / gal->boxsize[n] *
                              gal->boxsize_flatx[n - 1];
    }
    if (gal->boxsize[n] * gal->boxsize_flaty[n] >
        gal->boxsize[n - 1] * gal->boxsize_flaty[n - 1]) {
      gal->boxsize_flaty[n] = 0.95 * gal->boxsize[n - 1] / gal->boxsize[n] *
                              gal->boxsize_flaty[n - 1];
    }
    if (gal->boxsize[n] * gal->boxsize_flatz[n] >
        gal->boxsize[n - 1] * gal->boxsize_flatz[n - 1]) {
      gal->boxsize_flatz[n] = 0.95 * gal->boxsize[n - 1] / gal->boxsize[n] *
                              gal->boxsize_flatz[n - 1];
    }
    gal->ngrid[n][0] =
        (int)(gal->boxsize[n] * gal->boxsize_flatx[n] / gal->dx[n]);
    gal->ngrid[n][1] =
        (int)(gal->boxsize[n] * gal->boxsize_flaty[n] / gal->dx[n]);
    gal->ngrid[n][2] =
        (int)(gal->boxsize[n] * gal->boxsize_flatz[n] / gal->dx[n]);
  ...
  for (i = 0; i < AllVars.MaxCompNumber; i++) {
    if (gal->comp_type[i] == 0 &&
        (gal->comp_npart[i] > 0 || gal->comp_part_mass[i] > 0.) &&
        gal->comp_spherical_hydro_eq[i] == 0 && gal->comp_hydro_eq[i] > 0 &&
        gal->hydro_eq_niter > 0 && gal->comp_hydro_eq_mode[i] == 1) {
      // Allocate mdiplane density grid
      if (allocate_galaxy_midplane_dens(gal) != 0) {
        fprintf(stderr,
                "[Error] Unable to allocate midplane gas density grid\n");
        return -1;
      }
      break;
    }
    if (gal->hydro_eq_niter == 0)
      gal->comp_hydro_eq[i] = 0;
  }
  ...
  for (i = 0; i < AllVars.MaxCompNumber; i++) {
    ...
    if (gal->comp_npart[i] > 0 || gal->comp_part_mass[i] > 0.)
      effective_mass_factor = effective_mass_factor + gal->comp_mass_frac[i];
    // Disk fraction - count only flat components
    if (gal->comp_flatz[i] < 0.4 && gal->comp_type[i] != 1 &&
        (gal->comp_npart[i] > 0 || gal->comp_part_mass[i] > 0))
      gal->m_d += gal->comp_mass_frac[i];
    // Find the main halo
    if (gal->comp_type[i] == 1 && gal->index_halo == -1)
      gal->index_halo = i;
    if (gal->comp_type[i] == 2 && gal->comp_flatz[i] < 0.4 &&
        gal->index_disk == -1)
      gal->index_disk = i;
    if (gal->comp_type[i] == 0 && gal->comp_flatz[i] < 0.4 &&
        gal->index_gasdisk == -1)
      gal->index_gasdisk = i;
    if (gal->index_halo > -1)
      if (gal->comp_type[i] == 1 &&
          gal->comp_mass_frac[gal->index_halo] > gal->comp_mass_frac[i])
        gal->index_halo = i;
    if (gal->index_disk > -1)
      if (gal->comp_type[i] == 2 && gal->comp_flatz[i] < 0.4 &&
          gal->comp_mass_frac[gal->index_disk] > gal->comp_mass_frac[i])
        gal->index_disk = i;
    if (gal->index_gasdisk > -1)
      if (gal->comp_type[i] == 0 && gal->comp_flatz[i] < 0.4 &&
          gal->comp_mass_frac[gal->index_gasdisk] > gal->comp_mass_frac[i])
        gal->index_gasdisk = i;
  }
  // Default halo cut is r200
  if (gal->index_halo > -1)
    if (gal->comp_cut[gal->index_halo] <= 0.)
      gal->comp_cut[gal->index_halo] = gal->r200; //gjy note: default cut, but...

  // Allocate and set the gaussian field grid if necessary
  if (gal->dens_fluct_sigma > 0. && gal->dens_fluct_scale_inj > 0. &&
      dens_fluct > 0) {
    printf("这里这里这里这里\n\n\n\n\n"); //gjy add
    ...
    for (i = 0; i < AllVars.MaxCompNumber; i++) {
      if (gal->comp_type[i] == 0) {
        gal->dx_gauss = 2.0 *
                        (gal->comp_cut[i] + 5 * gal->comp_sigma_cut[i] *
                                                gal->comp_scale_length[i]) /
                        ((double)gal->ngrid_gauss[0]);
      }
    }
    printf("/////\t--------------------------------------------------\n");
    printf("/////\tSetting density fluctuations \n");
    printf("/////\t\t[sigma=%.2lf][scale inj=%.2le %s][scale diss=%.2le "
           "%s][spectral index=%.2lf]\n",
           gal->dens_fluct_sigma, gal->dens_fluct_scale_inj,
           AllVars.UnitLengthName, gal->dens_fluct_scale_diss,
           AllVars.UnitLengthName, gal->dens_fluct_nspec);
    printf("/////\t\t[grid scale=%.2le %s][seed=%ld]\n", gal->dx_gauss,
           AllVars.UnitLengthName, gal->dens_fluct_seed);
    if (gal->dx_gauss == 0.)
      printf("/////\t[Warning] a cut value has to be specified explicitely\n");
    fflush(stdout);
    set_galaxy_random_field_grid(gal, gal->dens_fluct_scale_inj,
                                 gal->dens_fluct_scale_diss,
                                 gal->dens_fluct_nspec, gal->dens_fluct_seed);
    if (gal->dens_fluct_sigma > 0.5) {
      printf("/////\t[Warning] Gaussian fluctuations may break the axisymmetry "
             "hypothesis\n");
    }
    printf("/////\t--------------------------------------------------\n");
  }
  ...
  for (i = 0; i < AllVars.MaxCompNumber; i++) {
    ...
    // printf("密度密度dice_init0: gal->comp_scale_dens[component] = %f\n", gal->comp_scale_dens[i]); //gjy add
    // Total number of particules
    if (gal->comp_mass_frac[i] == 0. && gal->comp_part_mass[i] == 0.)
      gal->comp_npart[i] = 0;
    if (gal->comp_mass_frac[i] == 0. && gal->comp_part_mass[i] > 0.)
      gal->comp_mass_frac[i] =
          gal->comp_npart[i] * gal->comp_part_mass[i] * (solarmass / unit_mass);
    if (gal->comp_npart_pot[i] < gal->comp_npart[i])
      gal->comp_npart_pot[i] = gal->comp_npart[i];
    if (gal->comp_npart[i] == 0 && gal->comp_part_mass[i] == 0.)
      gal->comp_npart_pot[i] = 0;
    // Rescale the mass fractions
    if (AllVars.NormMassFact == 1)
      gal->comp_mass_frac[i] = gal->comp_mass_frac[i] / effective_mass_factor;
    // Set the angular momentum fraction if not defined
    if (gal->comp_angmom_frac[i] == -1.0)
      gal->comp_angmom_frac[i] = gal->comp_mass_frac[i];
    ...
    if (gal->comp_scale_length[i] == 0. &&
        (gal->comp_type[i] == 2 || gal->comp_type[i] == 0) &&
        gal->comp_flatz[i] < 0.4 &&
        (gal->comp_npart[i] > 0 || gal->comp_part_mass[i] > 0.)) {
      // gal->comp_scale_length[i] =
      // disk_scale_length_func(gal,gal->comp_concentration[gal->index_halo],i);
      // Use Dutton+2011 Mass size relation for the disk scalelength assuming an
      // exponential profile
      gal->comp_scale_length[i] = disk_scale_length_obs_func(gal, i);
    }
    if (gal->comp_scale_length[i] == 0. && gal->comp_type[i] == 3 &&
        gal->comp_flatz[i] > 0.4 &&
        (gal->comp_npart[i] > 0 || gal->comp_part_mass[i] > 0.)) {
      if (gal->index_disk > -1) {
        // Compute the disk scale length if it has not been done yet
        if (gal->comp_scale_length[gal->index_disk] == 0.)
          gal->comp_scale_length[gal->index_disk] =
              disk_scale_length_obs_func(gal, gal->index_disk);
        gal->comp_scale_length[i] =
            0.333 * gal->comp_scale_length[gal->index_disk];
      }
    }
    ...
    if (gal->comp_concentration[i] <= 0. && gal->comp_scale_length[i] <= 0.) {
      gal->comp_concentration[i] =
          halo_concentration(gal->m200, AllVars.redshift);
      gal->comp_scale_length[i] = gal->r200 / gal->comp_concentration[i];
    }
    ...
    if (gal->comp_jeans_anisotropy_model[i] < 0 ||
        gal->comp_jeans_anisotropy_model[i] > 2) {
      fprintf(stderr,
              "\n[Error] Invalid value for jeans_anisotropy_model%d (valid = "
              "[0,1,2])\n",
              i + 1);
      exit(0);
    }
    ...
    if (gal->comp_imf_model[i] > 0 &&
        (gal->comp_type[i] == 0 || gal->comp_type[i] == 1))
      gal->comp_imf_model[i] = 0;
    if (gal->comp_imf_model[i] > 0) {
      // Particle mass cannot be defined
      gal->comp_part_mass[i] = 0.;
      // Assume the maximum number of particles
      gal->comp_npart[i] =
          (unsigned long int)(gal->comp_mass[i] /
                              (gal->comp_mstar_min[i] * solarmass / unit_mass));
      gal->comp_npart_pot[i] = gal->comp_npart[i];
    }
    // Set the start index for each component
    if (i > 0)
      gal->comp_start_part[i] =
          gal->comp_start_part[i - 1] + gal->comp_npart_pot[i - 1];
    // No anisotropy for gas component for the isotropic 1D Jeans equation
    if (gal->comp_type[i] == 0)
      gal->comp_jeans_anisotropy_model[i] = 0;
    if (gal->comp_cut[i] <= 0.) {
    ...
    if (gal->comp_softening[i] == 0.0)
      gal->comp_softening[i] = 0.05 * gal->comp_scale_length[i];
    // Default cut shape follows the galaxy component shape
    if (gal->comp_flatx_cut[i] == -1.0)
      gal->comp_flatx_cut[i] = gal->comp_flatx[i];
    if (gal->comp_flaty_cut[i] == -1.0)
      gal->comp_flaty_cut[i] = gal->comp_flaty[i];
    if (gal->comp_flatz_cut[i] == -1.0)
      gal->comp_flatz_cut[i] = gal->comp_flatz[i];
    if (gal->comp_flatx_out[i] == -1.0)
      gal->comp_flatx_out[i] = gal->comp_flatx[i];
    if (gal->comp_flaty_out[i] == -1.0)
      gal->comp_flaty_out[i] = gal->comp_flaty[i];
    if (gal->comp_flatz_out[i] == -1.0)
      gal->comp_flatz_out[i] = gal->comp_flatz[i];
    if (gal->comp_cut_hydro_eq[i] == 0.0)
      gal->comp_cut_hydro_eq[i] = gal->comp_cut[i];
    ...
    if (gal->comp_flatx[i] == gal->comp_flaty[i] && !gal->comp_dens_fluct[i]) {
      // Shperical symmetry
      if (gal->comp_flatz[i] == gal->comp_flatx[i] &&
          gal->comp_flatz[i] == gal->comp_flaty[i]) {
    ...
    if (gal->comp_spiral_r_in[i] == 0.)
      gal->comp_spiral_r_in[i] = 0.2 * gal->comp_scale_length[i];
    if (gal->comp_spiral_r_out[i] == 0.)
      gal->comp_spiral_r_out[i] = 0.8 * gal->comp_scale_length[i];
    ...
    if (gal->comp_part_mass[i] > 0. && gal->comp_npart[i] == 0) {
      gal->comp_npart[i] =
          (int)round(gal->comp_mass[i] /
                     (gal->comp_part_mass[i] * (solarmass / unit_mass)));
    }
    if (gal->comp_part_mass_pot[i] > 0. &&
        gal->comp_part_mass_pot[i] < gal->comp_part_mass[i] &&
        gal->comp_npart_pot[i] == 0) {
      gal->comp_npart_pot[i] =
          (int)round(gal->comp_mass[i] /
                     (gal->comp_part_mass_pot[i] * (solarmass / unit_mass)));
    }
    if (gal->comp_npart_pot[i] < gal->comp_npart[i])
      gal->comp_npart_pot[i] = gal->comp_npart[i];
    ...
    // Count stars
    if (gal->comp_type[i] > 1)
      gal->ntot_part_stars += gal->comp_npart[i];
    ...
    if (gal->comp_npart[i] > 0) {
      ...
      gal->n_component++;
      printf("%2d ", i + 1);
      // printf("boooooooooooooooooooooooooooooooooooooool\n\n\n\n\n\n"); //gjy add //this if
      fflush(stdout);
    } else
      gal->comp_bool[i] = 0;
    ...
    if (gal->comp_type[i] == 0) {
      if (gal->comp_thermal_eq[i] == 1 && gal->comp_hydro_eq[i] == 1) {
        fprintf(
            stderr,
            "[Error] hydro_eq%d and thermal_eq%d cannot be both equal to 1\n",
            i + 1, i + 1);
        return -1;
      }
      ...
      if (gal->comp_hydro_eq[i])
        gal->hydro_eq += gal->comp_hydro_eq[i] * gal->comp_bool[i];
      // Polytropic EoS case
      // The normalisation factor K is computed for T_init and dens_init
      gal->comp_k_poly[i] =
          gal->comp_u_init[i] *
          pow(gal->comp_dens_init[i] / unit_nh, 1.0 - gal->comp_gamma_poly[i]) *
          gamma_minus1;
      gal->comp_cs_init[i] =
          sqrt(gal->comp_k_poly[i] * gal->comp_gamma_poly[i] *
               pow(gal->comp_dens_init[i], gal->comp_gamma_poly[i] - 1.0));
      if (gal->comp_rc_entropy[i] == 0.)
        gal->comp_rc_entropy[i] = gal->comp_scale_length[i];
      if (gal->comp_alpha_entropy[i] < 0.)
        gal->comp_alpha_entropy[i] = 0.0;
    }
    ...
    if (gal->comp_bool[i]) {
      ...
      if (gal->comp_radius_nfw[i] == -1.0) {
        ...
        if (i == gal->index_halo && gal->comp_cut[i] != gal->r200) {
          ...
          printf("密度密度密度密度dice_init1(not else): \n\n\n\n\ngal->comp_scale_dens[component] = %f\n", gal->comp_scale_dens[i]); //gjy add
          printf("质量前: %f, %f, %f", gal->comp_mass[i], gal->comp_cut[i], cumulative_mass_func(gal, 2.0 * gal->comp_cut[i], i));

          gal->comp_scale_dens[i] = gal->comp_mass[i] /
              cumulative_mass_func(gal, 2.0 * gal->comp_cut[i], i);
          printf("质量后: gal->comp_scale_dens[component] = %f\n", gal->comp_scale_dens[i]); //gjy add
          printf("%f, %f, %f\n", gal->comp_mass[i], gal->comp_cut[i], cumulative_mass_func(gal, 2.0 * gal->comp_cut[i], i));
          printf("gal->r200=%f, gal->total_mass_r200=%f\n", gal->r200, gal->total_mass_r200);
          ...
          //gjy note: gal->comp_mass[component] is virial mass of this component //at the first, dens=1.
          printf("密度密度密度密度dice_init2: \n\n\n\n\ngal->comp_scale_dens[component] = %f\n", gal->comp_scale_dens[i]);
          printf("质量前: %f, %f, %f\n", gal->comp_mass[i], gal->comp_cut[i], cumulative_mass_func(gal, 2.0 * gal->comp_cut[i], i)); //gjy add //rho_crit \approx 1.398e-08

          gal->comp_scale_dens[i] = gal->comp_mass[i] /
              cumulative_mass_func(gal, 2.0 * gal->comp_cut[i], i); //gjy note: after changing comp_scale_dens of gal, cumulative_... will change, the later will not be true.
          printf("质量后: gal->comp_scale_dens[component] = %f\n", gal->comp_scale_dens[i]);
          printf("%f, %f, %f\n", gal->comp_mass[i], gal->comp_cut[i], cumulative_mass_func(gal, 2.0 * gal->comp_cut[i], i)); //gjy add //rho_crit \approx 1.398e-08
          printf("gal->r200=%f, gal->total_mass_r200=%f\n", gal->r200, gal->total_mass_r200);
          printf("多少多少多少多少: \n\n\n\n\n\ngAllVars.somewhat = %f\n", AllVars.h);
          ...
          if (gal->comp_excavate[i] > 0)
            gal->comp_excavate[i] = save;
        }
      } else {
        // Case where the final cutted mass is defined by a scaling with respect
        // to a NFW halo density
        //gjy note: this has 200*rho_crit
        m_scale_nfw =
            gal->comp_radius_nfw[i] / (gal->r200 / gal->comp_concentration[i]);
        delta_c =
            ((200. / 3.) * pow(gal->comp_concentration[i], 3.0)) /
            (log(1.0 + gal->comp_concentration[i]) -
             gal->comp_concentration[i] / (1.0 + gal->comp_concentration[i]));
        ...
        for (n = 0; n < 1000; n = n + 1) {
          mean_dens += density_functions_pool(gal, gal->comp_radius_nfw[i],
                                              (2.0 * pi * n / 1000.), 0., 0,
                                              gal->comp_model[i], i);
        }
        mean_dens /= 1000.;
        ...
        printf("密度密度密度密度dice_init3: \n\n\n\n\ngal->comp_scale_dens[component] = %f\n", gal->comp_scale_dens[i]); //gjy add
        printf("质量: \n\n\n\n%f, %f, %f", gal->comp_radius_nfw[i], rho_scale_nfw, mean_dens); //gjy add //rho_crit \approx 1.398e-08
      }
      // Turn on density cut
      if (cut_dens > 0.)
    ...
    // If it is the case, compute the disk scalelength according to Mo et al.
    // 1998
    if (gal->comp_type[i] == 0 && gal->comp_scale_length[i] > max_gas_radius)
      max_gas_radius = gal->comp_scale_length[i];
    if (gal->comp_type[i] == 1 && gal->comp_stream_method[i] == 0)
      gal->comp_stream_frac[i] =
          f_s_func(gal->comp_concentration[i], gal->lambda);
    // Checking total cutted mass
    if (gal->comp_type[i] == 0)
      cutted_gas_mass += gal->comp_bool[i] * gal->comp_mass[i];
    if (gal->comp_type[i] == 1)
      cutted_halo_mass += gal->comp_bool[i] * gal->comp_mass[i];
    if (gal->comp_type[i] == 2)
      cutted_disk_mass += gal->comp_bool[i] * gal->comp_mass[i];
    if (gal->comp_type[i] == 3)
      cutted_bulge_mass += gal->comp_bool[i] * gal->comp_mass[i];
    if (gal->comp_type[i] == 0 && gal->comp_cut[i] > gal->r200) {
      cutted_gas_mass_r200 =
          cutted_gas_mass_r200 + gal->comp_bool[i] * mass_r200;
    }
    if (gal->comp_type[i] == 0 && gal->comp_cut[i] <= gal->r200) {
      cutted_gas_mass_r200 =
          cutted_gas_mass_r200 + gal->comp_bool[i] * gal->comp_mass[i];
    }
    ...
    if (gal->comp_type[i] == 0)
      gal->num_part[0] += gal->comp_npart[i];
    if (gal->comp_type[i] == 1)
      gal->num_part[1] += gal->comp_npart[i];
    if (gal->comp_type[i] == 2)
      gal->num_part[2] += gal->comp_npart[i];
    if (gal->comp_type[i] == 3)
      gal->num_part[3] += gal->comp_npart[i];
    if (gal->comp_type[i] == 0)
      gal->num_part_pot[0] += gal->comp_npart_pot[i];
    if (gal->comp_type[i] == 1)
      gal->num_part_pot[1] += gal->comp_npart_pot[i];
    if (gal->comp_type[i] == 2)
      gal->num_part_pot[2] += gal->comp_npart_pot[i];
    if (gal->comp_type[i] == 3)
      gal->num_part_pot[3] += gal->comp_npart_pot[i];
  }
  for (i = 0; i < AllVars.MaxCompNumber; i++) {
    if (gal->comp_metal[i] == -1.0)
      gal->comp_metal[i] = galaxy_metallicity(
          (cutted_bulge_mass + cutted_disk_mass), AllVars.redshift);
  }
  // Total angular momentum of the halo assuming NFW profile
  if (gal->index_halo > -1) {
    gal->J200 =
        gal->lambda * sqrt(2.0 * G * pow(gal->m200, 3) * gal->r200 /
                           f_c_func(gal->comp_concentration[gal->index_halo]));
  } else {
    gal->J200 = 0.;
  }
  ...
  for (i = 0; i < AllVars.MaxCompNumber; i++) {
    if (gal->comp_npart[i] > 0 && gal->comp_mass[i] > 0. &&
        gal->comp_compute_vel[i] == 1) {
      gal->index_first = i;
      break;
    }
  }
  ...
  baryonic_fraction =
      (cutted_disk_mass + cutted_gas_mass + cutted_bulge_mass) / gal->m200;
  BT_fraction = cutted_bulge_mass / (cutted_bulge_mass + cutted_disk_mass);
  BD_fraction = cutted_bulge_mass / cutted_disk_mass;
  gas_fraction = cutted_gas_mass /
                 (cutted_disk_mass + cutted_bulge_mass + cutted_gas_mass);

  // Print some information to screen.
  if (info != 0) {
    printf("/////\tVirial quantities [z=%5.2lf]\n", AllVars.redshift);
    printf("/////\t\t- V200 =  %7.1lf %s\n", gal->v200,
           AllVars.UnitVelocityName);
    printf("/////\t\t- R200 =  %7.1lf %s  [  %7.1lf %s.h^-1  ]\n", gal->r200,
           AllVars.UnitLengthName, gal->r200 * AllVars.h,
           AllVars.UnitLengthName);
    printf("/////\t\t- M200 = %6.2le Msol [ %6.2le Msol.h^-1 ]\n",
           gal->m200 * unit_mass / solarmass,
           gal->m200 * AllVars.h * unit_mass / solarmass);
    printf("/////\t\t- J200 = %6.2le Msol.%s.%s\n",
           gal->J200 * unit_mass / solarmass, AllVars.UnitLengthName,
           AllVars.UnitVelocityName);
    ...
    printf("/////\t\t- Halo          mass \t-> %10.2le Msol [%5.2lf%%]\n",
           cutted_halo_mass * unit_mass / solarmass,
           100. * cutted_halo_mass / gal->total_mass_r200);
    if (cutted_halo_mass > 1.01 * cutted_halo_mass_r200) {
      printf("/////\t\t- Halo(r<R200)  mass \t-> %10.2le Msol [%5.2lf%%]\n",
             cutted_halo_mass_r200 * unit_mass / solarmass,
             100. * cutted_halo_mass_r200 / gal->total_mass_r200);
    }
    printf("/////\t\t- Disk          mass \t-> %10.2le Msol [%5.2lf%%]\n",
           cutted_disk_mass * unit_mass / solarmass,
           100. * cutted_disk_mass / gal->total_mass_r200);
    printf("/////\t\t- Gas           mass \t-> %10.2le Msol [%5.2lf%%]\n",
           cutted_gas_mass * unit_mass / solarmass,
           100. * cutted_gas_mass / gal->total_mass_r200);
    if (cutted_gas_mass > 1.01 * cutted_gas_mass_r200) {
      printf("/////\t\t- Gas(r<R200)   mass \t-> %10.2le Msol [%5.2lf%%]\n",
             cutted_gas_mass_r200 * unit_mass / solarmass,
             100. * cutted_gas_mass_r200 / gal->total_mass_r200);
    }
    printf("/////\t\t- Bulge         mass \t-> %10.2le Msol [%5.2lf%%]\n",
           cutted_bulge_mass * unit_mass / solarmass,
           100. * cutted_bulge_mass / gal->total_mass_r200);
    printf("/////\t\t- Stellar       mass \t-> %10.2le Msol [%5.2lf%%] \n",
           (cutted_bulge_mass + cutted_disk_mass) * unit_mass / solarmass,
           100. * (cutted_bulge_mass + cutted_disk_mass) /
               gal->total_mass_r200);
    printf("/////\t\t- Abundance matching \t-> %10.2le Msol [%5.2lf%%] \n",
           halo_abundance(gal->m200, AllVars.redshift) * unit_mass / solarmass,
           100. * halo_abundance(gal->m200, AllVars.redshift) /
               gal->total_mass_r200);
    ...
    if (gal->num_part[0] > 0 && gal->num_part[2] > 0)
      printf("/////\t\t- Gas fraction\t\t-> %10.3lf\n", gas_fraction);
    if (gal->num_part[2] > 0 && gal->num_part[3] > 0)
      printf("/////\t\t- Bulge Disk fraction\t-> %10.3lf\n", BD_fraction);
    if (gal->num_part[2] > 0 && gal->num_part[3] > 0)
      printf("/////\t\t- Bulge Total fraction\t-> %10.3lf\n", BT_fraction);
    printf("/////\t--------------------------------------------------\n");
    printf("/////\tGrids \n");
    for (i = 0; i < gal->nlevel; i++) {
      printf("/////\t\t- Level %d \t\t\t[ %4d,%4d,%4d ][box=%6.1lf "
             "%s][dx=%8.3le %s]\n",
             i + gal->level_coarse, gal->ngrid[i][0], gal->ngrid[i][1],
             gal->ngrid[i][2], gal->boxsize[i], AllVars.UnitLengthName,
             gal->dx[i], AllVars.UnitLengthName);
    }
    printf("/////\t\t------------------------------------------\n");
    for (i = 0; i < AllVars.MaxCompNumber; i++) {
      if (gal->midplane_dens_defined == 1) {
        printf("/////\t\t- Midplane gas density \t\t[   %4d,%4d    ]\n",
               (int)pow(2, gal->level_grid_dens),
               (int)pow(2, gal->level_grid_dens));
        break;
      }
    }
    if (gal->dens_fluct_sigma > 0. && gal->dens_fluct_scale_inj > 0. &&
      ...
      printf("/////\t\t- Gaussian field density \t[ %4d,%4d,%4d ]\n",
             (int)pow(2, gal->level_grid_dens_fluct),
             (int)pow(2, gal->level_grid_dens_fluct),
             (int)pow(2, gal->level_grid_dens_fluct));
    }
    for (i = 0; i < AllVars.MaxCompNumber; i++) {
    ...
    if (gal->level_grid_jeans_3D > 0 && gal->jeans_3D_defined) {
      printf("/////\t\t- Jeans mixed moments \t\t[   %4d,%4d    ][box=%6.1lf "
             "%s][dx=%8.3le %s]\n",
             gal->ngrid_jeans[0], gal->ngrid_jeans[1], gal->boxsize_jeans,
             AllVars.UnitLengthName, gal->dx_jeans, AllVars.UnitLengthName);
    }
  }
  fflush(stdout);
  ...
  for (j = 0; j < AllVars.MaxCompNumber; j++) {
    if (gal->comp_imf_model[j] > 0 && gal->comp_npart[j] > 1) {
      ...
      double junk = imf_functions_pool(
          gal, gal->comp_mstar_min[j] * solarmass / unit_mass,
          gal->comp_imf_model[j], j);
      ...
      double mtot = 0.;
      for (k = gal->comp_start_part[j];
           k < gal->comp_start_part[j] + gal->comp_npart_pot[j]; k++) {
        mtot += gal->mass[k];
        // Stop when the total mass of the component is reached
        if (mtot > gal->comp_mass[j])
          break;
      }
  ...
  for (j = 0; j < AllVars.MaxCompNumber; j++) {
    if (gal->comp_npart[j] > 0) {
      if (gal->comp_sfr[j] <= 0.) {
        gal->comp_sfr[j] =
            150. * pow(0.1 * (cutted_bulge_mass + cutted_disk_mass), 0.8) *
            pow((1.0 + AllVars.redshift) / 3.2, 2.7);
      }
      if (gal->comp_sfr[j] != 0.)
        gal->comp_mean_age[j] = (gal->comp_mass[j] * (unit_mass / solarmass)) /
                                (gal->comp_sfr[j] * 1e6) / 2.0;
      printf("/////\t------------------ Component %2d ------------------\n",
             j + 1);
      printf("/////\t\t-> np        = %ld \n", gal->comp_npart[j]);
      if (gal->comp_npart_pot[j] > gal->comp_npart[j])
      ...
      printf("/////\t\t-> cut       = %6.2le [%s]\n", gal->comp_cut[j],
             AllVars.UnitLengthName);
      printf("/////\t\t-> softening = %6.2le [%s]\n", gal->comp_softening[j],
             AllVars.UnitLengthName);
      printf("/////\t\t-> f_stream  = %6.3le\n", gal->comp_stream_frac[j]);
      if (gal->comp_jeans_dim[j] > 0)
        printf("/////\t\t-> jeans dim = %d \n", gal->comp_jeans_dim[j]);
      if (gal->comp_cut_in[j] > 0)
        printf("/////\t\t-> cut_in    = %6.2le [%s]\n", gal->comp_cut_in[j],
               AllVars.UnitLengthName);
      ...
      if (gal->comp_type[j] == 0) {
        ...
        printf("/////\t\t-> S_init    = %6.2le [keV.cm^2]\n",
               boltzmann_kev * gal->comp_t_init[j] *
                   pow(gal->comp_dens_init[j] / unit_nh * unit_ne, -2.0 / 3.0));
        printf("/////\t\t-> rho_init  = %6.2le [H/cm^3]\n",
               gal->comp_dens_init[j]);
        printf("/////\t\t-> gamma     = %6.2le \n", gal->comp_gamma_poly[j]);
        printf("/////\t\t-> cs        = %6.2le [%s]\n", gal->comp_cs_init[j],
               AllVars.UnitVelocityName);
        if (gal->comp_cut_dens[j] > 0)
          printf("/////\t\t-> cut_dens  = %6.2le [%s %s^-3]\n",
                 gal->comp_cut_dens[j] / unit_nh, AllVars.UnitMassName,
                 AllVars.UnitLengthName);
      }
      if (gal->comp_type[j] > 1) {
    ...
    if (gal->comp_imf_model[j] == 0) {
      ...
      for (k = gal->comp_start_part[j];
        ...
        gal->age[k] = -(2 * gal->comp_mean_age[j] * gsl_rng_uniform_pos(r[0]) +
                        gal->comp_min_age[j]);
      }
    }
  }
  ...
  nt = 0;
  for (j = 0; j < AllVars.MaxCompNumber; j++) {
    if (gal->comp_type[j] > 1 && gal->comp_age_sigma[j] > 0. &&
        gal->comp_age_scale[j] > 0. && gal->comp_npart[j] > 0 &&
      ...
      set_galaxy_random_field_grid(gal, gal->comp_age_scale[j],
                                   gal->comp_age_scale[j], 1.0,
                                   gal->comp_age_seed[j]);

      mean_age = 0.;
      max_age = 0.;
      printf(
          "/////\t\t- Component %2d -> setting age fluctuations [sigma=%.2lf "
          "Myr][scale=%.2lf kpc][grid scale=%.3lf kpc][seed=%ld]\n",
          j + 1, gal->comp_age_sigma[j], gal->comp_age_scale[j], gal->dx_gauss,
          gal->comp_age_seed[j]);
      for (k = gal->comp_start_part[j];
           k < gal->comp_start_part[j] + gal->comp_npart_pot[j]; k++) {
        gal->age[k] +=
            galaxy_gaussian_field_func(gal, gal->x[k], gal->y[k], gal->z[k]) *
            gal->comp_age_sigma[k];
        if (gal->age[k] > max_age)
          max_age = gal->age[k];
      }
  ...
  nt = 0;
  for (j = 0; j < AllVars.MaxCompNumber; j++) {
    if (gal->comp_metal_sigma[j] > 0. && gal->comp_metal_scale[j] > 0. &&
        gal->comp_npart[j] > 0 && gal->comp_metal[j] != 0.) {
      double mean_metal;
      nt++;
      if (nt == 1) {
        printf("/////\t--------------------------------------------------\n");
      ...
      set_galaxy_random_field_grid(gal, gal->comp_metal_scale[j],
                                   gal->comp_metal_scale[j], 1.0,
                                   gal->comp_metal_seed[j]);

      mean_metal = 0.;
      printf("/////\t\t- Component %2d -> setting metal fluctuations "
             "[sigma=%.2lf][scale=%.2le %s][grid scale=%.2le %s][seed=%ld]\n",
             j + 1, gal->comp_metal_sigma[j], gal->comp_metal_scale[j],
             AllVars.UnitLengthName, gal->dx_gauss, AllVars.UnitLengthName,
             gal->comp_metal_seed[j]);
      for (k = gal->comp_start_part[j];
           k < gal->comp_start_part[j] + gal->comp_npart_pot[j]; k++) {
        gal->metal[k] +=
            galaxy_gaussian_field_func(gal, gal->x[k], gal->y[k], gal->z[k]) *
            gal->comp_metal_sigma[k];
        if (gal->metal[k] < 0)
          gal->metal[k] = 0.;
...
int create_stream(stream *st, char *fname, int info) {
  ...
  // Allocate and set the gaussian field grid if necessary
  if (st->dens_fluct_sigma > 0. && st->dens_fluct_scale_inj > 0. &&
      dens_fluct > 0) {
    ...
    st->dx_gauss = 0.;
    for (i = 0; i < AllVars.MaxCompNumber; i++) {
      if (2.1 * st->comp_length[i] / ((double)st->ngrid_gauss[0]) >
          st->dx_gauss)
        st->dx_gauss = 2.1 * st->comp_length[i] / ((double)st->ngrid_gauss[0]);
    }

    printf("/////\t--------------------------------------------------\n");
    printf("/////\tSetting density fluctuations [sigma=%.2lf][scale inj=%.2le "
           "%s][scale diss=%.2le %s][spectral index=%.2lf]\n",
           st->dens_fluct_sigma, st->dens_fluct_scale_inj,
           AllVars.UnitLengthName, st->dens_fluct_scale_diss,
           AllVars.UnitLengthName, st->dens_fluct_nspec);
    printf("/////\t                             [grid scale=%.3lf "
           "kpc][seed=%ld]\n",
           st->dx_gauss, st->dens_fluct_seed);
    fflush(stdout);
    set_stream_random_field_grid(st, st->dens_fluct_scale_inj,
                                 st->dens_fluct_scale_diss,
                                 st->dens_fluct_nspec, st->dens_fluct_seed);
    printf("/////\t--------------------------------------------------\n");
  }
  ...
  for (i = 0; i < AllVars.MaxCompNumber; i++) {
    ...
    // Set the start index for each component
    if (i > 0)
      st->comp_start_part[i] =
          st->comp_start_part[i - 1] + st->comp_npart[i - 1];
    ...
    if (st->comp_bool[i]) {
      base = 2 * st->comp_length[i] *
             atan(pi / 180. * st->comp_opening_angle[i] / 2.);
      ...
      st->comp_k_poly[i] =
          st->comp_u_init[i] *
          pow(st->comp_dens[i] / unit_nh, 1.0 - st->comp_gamma_poly[i]) *
          gamma_minus1;
      st->comp_cs_init[i] =
          sqrt(st->comp_k_poly[i] * st->comp_gamma_poly[i] *
               pow(st->comp_dens[i], st->comp_gamma_poly[i] - 1.0));
    }
  }
  ...
  for (j = 0; j < AllVars.MaxCompNumber; j++) {
    if (st->comp_npart[j] > 0)
      printf("/////\t\t- Component %2d -> m=%.2e Msol\n", j + 1,
             (st->comp_mass[j] * 1.0E10) / (st->comp_npart[j]));
    // Filling the arrays of the &galaxy structure
    for (i = st->comp_start_part[j];
         i < st->comp_start_part[j] + st->comp_npart[j]; i++) {
...
int set_galaxy_coords(galaxy *gal) {
  ...
  for (i = 0; i < AllVars.MaxCompNumber; i++) {
    if (gal->comp_npart[i] > 0) {
      if (gal->comp_npart[i] > 1) {
        printf("/////\t\t- Component %2d [%s]", i + 1,
               gal->comp_profile_name[i]);
      } else {
  ...
  if (gal->hydro_eq > 0 && gal->hydro_eq_niter > 0) {
    ...
    for (j = 0; j < gal->hydro_eq_niter; j++) {
      printf("/////\t\tIteration [%2d/%2d][evaluating potential]", j + 1,
             gal->hydro_eq_niter);
      ...
      for (i = 0; i < AllVars.MaxCompNumber; i++) {
        if ((gal->comp_npart[i] > 0) &&
            (gal->comp_type[i] == 0) & (gal->comp_hydro_eq[i] == 1)) {
          // Put the component in the xy plane
          if (fabs(gal->comp_theta_sph[i]) > 0. ||
              fabs(gal->comp_phi_sph[i]) > 0.)
            rotate_all(gal, -gal->comp_theta_sph[i], -gal->comp_phi_sph[i], 2);
          // Only apply the density cut for the last iteration
          if (j < gal->hydro_eq_niter - 1 &&
              gal->comp_spherical_hydro_eq[i] == 0) {
            cut_dens = gal->comp_cut_dens[i];
            gal->comp_cut_dens[i] = 0.;
          }
          if (gal->comp_spherical_hydro_eq[i] == 1) {
            gal->comp_mass[i] =
                cumulative_mass_func(gal, 2.0 * gal->comp_cut[i], i);
            for (k = gal->comp_start_part[i];
                 k < gal->comp_start_part[i] + gal->comp_npart_pot[i]; k++) {
          ...
          gal->boxsize_dens =
              2.5 * (gal->comp_cut[i] +
                     3 * gal->comp_scale_length[i] * gal->comp_sigma_cut[i]);
          gal->dx_dens = gal->boxsize_dens / ((double)gal->ngrid_dens[0]);
          // Compute midplane density
          if (gal->comp_spherical_hydro_eq[i] == 0 &&
              gal->comp_hydro_eq_mode[i] == 1) {
            fill_midplane_dens_grid(gal);
          }
          printf("/////\t\t\t- Component %2d [   m_tot=%4.2le Msol   ]", i + 1,
                 gal->comp_mass[i] * unit_mass / solarmass);
          mcmc_metropolis_hasting_ntry(gal, i, gal->comp_model[i]);
          // Restore density cut
          if (j < gal->hydro_eq_niter - 1 &&
              gal->comp_spherical_hydro_eq[i] == 0) {
          ...
          if (fabs(gal->comp_theta_sph[i]) > 0. ||
              fabs(gal->comp_phi_sph[i]) > 0.)
            rotate_all(gal, gal->comp_theta_sph[i], gal->comp_phi_sph[i], 1);
        }
      }
    }
    for (i = 0; i < AllVars.MaxCompNumber; i++) {
      if (gal->comp_turb_frac[i] > 0) {
        ...
        if (gal->comp_turb_sigma[i] == 0. && gal->comp_turb_frac[i] > 0.) {
          gal->comp_turb_sigma[i] =
              gal->comp_cs_init[i] *
              sqrt(gal->comp_turb_frac[i] / (1.0 - gal->comp_turb_frac[i]));
        }
      }
    }
    for (i = 0; i < AllVars.Nthreads; i++)
...
// User can choose between cold/hot particles, ie. without velocity
// dispersion/with velocity dispersion This computation is inspired from the
// work of Springel et al. 2005 & Hernquist 1993
int set_galaxy_velocity(galaxy *gal) {
  ...
  double v_c, v_r, v_theta, v_phi, v_z, v_cmax, v_stream_max, Q, J_comp, J_sum,
      lambda_crit, eps_m;
  double v2a_r, v2a_theta, v2_theta, v2a_z, va_theta, sigma2_theta, vel_x,
      vel_y, vel_z, stddev_vx, stddev_vy, stddev_vz;
  ...
  for (i = 0; i < gal->ntot_part_pot; ++i) {
    if (fabs(gal->r_sph[i]) > gal->maxrad)
      gal->maxrad = fabs(gal->r_sph[i]);
  }
  for (j = 0; j < AllVars.MaxCompNumber; j++) {
    if (gal->comp_cut[j] > gal->maxrad && gal->comp_npart[j] > 1)
      gal->maxrad = gal->comp_cut[j];
    if (gal->comp_type[j] == 0) {
      for (i = gal->comp_start_part[j];
           i < gal->comp_start_part[j] + gal->comp_npart_pot[j]; ++i) {
        if (fabs(gal->r_sph[i]) > gal->maxrad_gas)
          gal->maxrad_gas = fabs(gal->r_sph[i]);
      }
    }
  }
  ...
  for (k = 0; k < AllVars.Ngal; k++) {
    if (AllVars.Circular_Gal1[k] > 0 && AllVars.Circular_Gal2[k] > 0 &&
        AllVars.Circular_GalCenter[k] - 1 == AllVars.CurrentGalaxy) {
      gal->index[tid] = 0;
      save1 = gal->z[gal->index[tid]];
      save2 = gal->theta_cyl[gal->index[tid]];
      gal->z[gal->index[tid]] = 0.;
  ...
  for (j = 0; j < AllVars.MaxCompNumber; j++) {
    ...
    if (gal->comp_sigmar_model[j] > 0) {
      ...
      gal->x[gal->index[tid]] =
          gal->r_cyl[gal->index[tid]] * cos(gal->theta_cyl[gal->index[tid]]);
      gal->y[gal->index[tid]] =
          gal->r_cyl[gal->index[tid]] * sin(gal->theta_cyl[gal->index[tid]]);
      gal->comp_model[j] = gal->comp_sigmar_model[j];
      gal->comp_sigmar_scale[j] =
          pow(gal->comp_sigmar[j], 2) /
          surface_density_func(gal, gal->r_cyl[i], gal->theta_cyl[i], 1, j);
      gal->z[gal->index[tid]] = save1;
      gal->theta_cyl[gal->index[tid]] = save2;
    ...
    if (gal->comp_sigmaz_model[j] > 0) {
      ...
      gal->comp_sigmaz_scale[j] =
          pow(gal->comp_sigmaz[j], 2) /
          surface_density_func(gal, gal->r_cyl[i], gal->theta_cyl[i], 1, j);
      gal->z[gal->index[tid]] = save1;
      gal->theta_cyl[gal->index[tid]] = save2;
      gal->r_cyl[gal->index[tid]] = save3;
    ...
    if (gal->comp_npart[j] > 1 && gal->comp_compute_vel[j] == 1) {
      ...
      if (gal->comp_theta_sph[j] != 0. || gal->comp_phi_sph[j] != 0.)
        rotate_all(gal, -gal->comp_theta_sph[j], -gal->comp_phi_sph[j], 2);
      if (j > gal->index_first &&
          (gal->comp_softening[j] != gal->comp_softening[j - 1] ||
           gal->comp_theta_sph[j] != 0. || gal->comp_phi_sph[j] != 0.)) {
      ...
      for (i = gal->comp_start_part[j];
        ...
        int index2 = index1 + 1;
        double cmass_interp =
            interpol(index1 * interval, index2 * interval, gal->r_sph[i],
                     cmass[index1], cmass[index2]);
        double vcirc_interp =
            interpol((index1 + 0.5) * interval, (index2 + 0.5) * interval,
                     gal->r_sph[i], vcirc[index1], vcirc[index2]);
        ...
        case 1:
          // Bullock 2001 method: specific angular momentum follows the
          // cumulative mass profile
          v_theta = gal->comp_stream_scale[j] * cmass_interp /
                    max(gal->r_sph[i], interval);
          break;
        case 2:
        ...
        gal->vel_x[i] =
            (v_r * cos(gal->theta_cyl[i]) - v_theta * sin(gal->theta_cyl[i]));
        gal->vel_y[i] =
            (v_r * sin(gal->theta_cyl[i]) + v_theta * cos(gal->theta_cyl[i]));
        gal->vel_z[i] = v_z;
      }
      // Compute the stream scale factor to ensure a consistent total angular
      // momentum
      J_comp = Jtot_func(gal, j);
      if (J_comp > 0.0)
        gal->comp_stream_scale[j] =
            gal->comp_angmom_frac[j] * gal->J200 / J_comp;
      ...
      if (gal->comp_stream_method[j] == 2)
        gal->comp_stream_frac[j] = gal->comp_stream_scale[j];
      fflush(stdout);
      ...
      if (gal->comp_turb_sigma[j] > 0. && gal->comp_npart[j] > 1 &&
        ...
        gal->dx_gauss = 2.1 * gal->comp_cut[j] / ((double)gal->ngrid_gauss[0]);
        gal->dx_gauss = 2.0 *
                        (gal->comp_cut[j] + 5 * gal->comp_sigma_cut[j] *
                                                gal->comp_scale_length[j]) /
                        ((double)gal->ngrid_gauss[0]);
        printf("\n/////\t\t- Component %2d [sigma_turb=%.2lf km/s][scale "
               "inj=%.2e %s][scale diss=%.2e %s][spectral index=%.2lf]\n",
               j + 1, gal->comp_turb_sigma[j], gal->comp_turb_scale_inj[j],
               AllVars.UnitLengthName, gal->comp_turb_scale_diss[j],
               AllVars.UnitLengthName, gal->comp_turb_nspec[j]);
        printf("/////\t\t ------------- [grid scale=%.2e %s][seed=%ld]\n",
               gal->dx_gauss, AllVars.UnitLengthName, gal->comp_turb_seed[j]);
        // Generating velocites
        set_galaxy_random_field_grid(
            gal, gal->comp_turb_scale_inj[j], gal->comp_turb_scale_diss[j],
            gal->comp_turb_nspec[j], gal->comp_turb_seed[j] + 0);
        for (i = gal->comp_start_part[j];
             i < gal->comp_start_part[j] + gal->comp_npart[j]; ++i) {
          gal->vel_x[i] +=
              galaxy_gaussian_field_func(gal, gal->x[i], gal->y[i], gal->z[i]);
        }
        set_galaxy_random_field_grid(
            gal, gal->comp_turb_scale_inj[j], gal->comp_turb_scale_diss[j],
            gal->comp_turb_nspec[j], gal->comp_turb_seed[j] + 1);
        for (i = gal->comp_start_part[j];
             i < gal->comp_start_part[j] + gal->comp_npart[j]; ++i) {
          gal->vel_y[i] +=
              galaxy_gaussian_field_func(gal, gal->x[i], gal->y[i], gal->z[i]);
        ...
        set_galaxy_random_field_grid(
            gal, gal->comp_turb_scale_inj[j], gal->comp_turb_scale_diss[j],
            gal->comp_turb_nspec[j], gal->comp_turb_seed[j] + 2);
        for (i = gal->comp_start_part[j];
             i < gal->comp_start_part[j] + gal->comp_npart[j]; ++i) {
          gal->vel_z[i] +=
              galaxy_gaussian_field_func(gal, gal->x[i], gal->y[i], gal->z[i]);
        ...
        stddev_vx = standard_dev(gal->vel_x, gal->comp_start_part[j],
                                 gal->comp_start_part[j] + gal->comp_npart[j]);
        stddev_vy = standard_dev(gal->vel_y, gal->comp_start_part[j],
                                 gal->comp_start_part[j] + gal->comp_npart[j]);
        stddev_vz = standard_dev(gal->vel_z, gal->comp_start_part[j],
                                 gal->comp_start_part[j] + gal->comp_npart[j]);
      ...
      if (gal->comp_type[j] == 0 && gal->hydro_eq_niter > 0 &&
...
#pragma omp parallel for private(v_c, v_r, v_theta, v_z, v2a_r, v2a_z, v2a_theta, v2_theta, va_theta, sigma2_theta, vel_x, vel_y, vel_z, Q, save) shared(j,gal) reduction(+:nrejected,nrejected_failed)
        for (i = gal->comp_start_part[j];
          ...
          case 1:
            // Bullock 2001 method: specific angular momentum follows the
            // cumulative mass profile
            va_theta = gal->comp_stream_frac[j] * gal->comp_stream_scale[j] *
                       cmass_interp / max(gal->r_sph[i], interval);
            break;
          case 2:
            // Springel 1999 method: the streaming velocity is a fixed fraction
          ...
          if (va_theta > v_stream_max && gal->comp_type[j] > 0)
            v_stream_max = va_theta;
          // Specific case of gas particles
          if (gal->comp_type[j] == 0) {
            v2_theta = v2_theta_gas_func(gal, gal->r_cyl[i], gal->z[i], j);
            // No velocity dispersion is needed because we are dealing with a
            // fluid ie. collisional particles
            v_r = 0.;
            ...
            if (v_theta > v_stream_max)
              v_stream_max = v_theta;
            // Cylindrical to cartesian coordinates
            gal->vel_x[i] += (v_r * cos(gal->theta_cyl[i]) -
                              v_theta * sin(gal->theta_cyl[i]));
            gal->vel_y[i] += (v_r * sin(gal->theta_cyl[i]) +
                              v_theta * cos(gal->theta_cyl[i]));
            gal->vel_z[i] += v_z;
          } else {
            ...
            if (gal->comp_jeans_dim[j] != 1) {
              ...
              case 2:
                ...
                v2a_z = v2a_r;
                v2a_theta =
                    v2a_theta_2D_func(gal, fabs(gal->r_cyl[i]), v2a_r, v_c, j);
                break;
              ...
              case 0:
                ...
                if (gal->comp_sigmar_model[j] == 0) {
                  v2a_r = pi * G *
                          surface_density_func(gal, gal->r_cyl[i],
                                               gal->theta_cyl[i], 1, j) *
                          gal->comp_scale_length[j] * gal->comp_flatz[j];
                  // Dispersion proportional to surface density
                } else if (gal->comp_sigmar_model[j] > 0) {
                  save = gal->comp_model[j];
                  gal->comp_model[j] = gal->comp_sigmar_model[j];
                  v2a_r = gal->comp_sigmar_scale[j] *
                          surface_density_func(gal, gal->r_cyl[i],
                                               gal->theta_cyl[i], 1, j);
                  gal->comp_model[j] = save;
                }
              ...
              if (gal->comp_epicycle[j] == 1) {
                sigma2_theta =
                    sigma2_theta_epicycle_func(gal, fabs(gal->r_cyl[i]), v2a_r);
                va_theta = v2a_theta >= sigma2_theta
                               ? sqrt(v2a_theta - sigma2_theta)
                               : 0.;
                // Enforce Q>Q_min
                if (gal->comp_Q_lim[j] > 0 || gal->comp_Q_fixed[j] > 0 ||
                    gal->comp_Q_boost[j] > 0) {
                  double v2a_r_new =
                      v2a_r_toomre(gal, fabs(gal->r_cyl[i]), v2a_r, j);
                  v2a_r = v2a_r_new;
                } else {
                ...
                if (gal->comp_k_stream[j] > 0.) {
                  ...
                  k_stream = gal->comp_k_stream[j] > kmax_stream
                                 ? kmax_stream
                                 : gal->comp_k_stream[j];
                  va_theta = v2a_theta >= v2a_r
                                 ? k_stream * sqrt(v2a_theta - v2a_r)
                                 : 0.;
                }
                // Check if the dispersion is a real or complex number
                sigma2_theta = (v2a_theta >= pow(va_theta, 2.0))
                                   ? (v2a_theta - pow(va_theta, 2.0))
                                   : 0.;
              }
              if (va_theta > v2a_theta)
                warning1 = 1;
              if (va_theta > v_stream_max)
                v_stream_max = va_theta;

              // Drawing random velocities
              v_r = gsl_ran_exppow(r[tid], sqrt(2.0 * v2a_r),
                                   gal->comp_ggd_beta[j]);
              v_theta = gsl_ran_exppow(r[tid], sqrt(2.0 * sigma2_theta),
                                       gal->comp_ggd_beta[j]) +
                        va_theta;
              v_z = gsl_ran_exppow(r[tid], sqrt(2.0 * v2a_z),
                                   gal->comp_ggd_beta[j]);
              ...
              if (gal->comp_vmax_esc[j] > 0.) {
                vmax =
                    fabs(gal->comp_vmax_esc[j]) *
                    sqrt(2 * fabs(galaxy_total_potential(
                                 gal, gal->x[i], gal->y[i], gal->z[i], 1, 0)));
              }
              ...
              if (vmax > 0.) {
                // Let's ensure that the particle velocity is lower than
                // gal->comp_vmax times the escape velocity
                int ct = 0;
                while (sqrt(pow(v_r, 2) + pow(v_theta, 2) + pow(v_z, 2)) >
                       vmax) {
                  if (ct >= AllVars.GaussianRejectIter) {
                    v_r = (2.0 / 3.0) * vmax *
                          (gsl_rng_uniform_pos(r[tid]) - 0.5);
                    v_theta = (2.0 / 3.0) * vmax *
                                  (gsl_rng_uniform_pos(r[tid]) - 0.5) +
                              va_theta;
                    v_z = (2.0 / 3.0) * vmax *
                          (gsl_rng_uniform_pos(r[tid]) - 0.5);
              ...
              // Cylindrical to cartesian coordinates
              vel_x = (v_r * cos(gal->theta_cyl[i]) -
                       v_theta * sin(gal->theta_cyl[i]));
              vel_y = (v_r * sin(gal->theta_cyl[i]) +
                       v_theta * cos(gal->theta_cyl[i]));
              vel_z = v_z;
              ...
              case 1:
                ...
                break;
              case 2:
                drhodr = deriv_central2(gal, radius,
                                        0.01 * gal->comp_scale_length[j],
                                        density_wrapper_func);
              ...
              v_theta = gsl_ran_exppow(r[tid], sqrt(2.0 * v2a_theta),
                                       gal->comp_ggd_beta[j]);
              v_phi = gsl_ran_exppow(r[tid], sqrt(2.0 * v2a_theta),
                                     gal->comp_ggd_beta[j]);
              ...
              if (vmax > 0.) {
                ...
                // gal->comp_vmax times the escape velocity
                int ct = 0;
                while (sqrt(pow(v_r, 2) + pow(v_theta, 2) + pow(v_phi, 2)) >
                       vmax) {
                  if (ct >= AllVars.GaussianRejectIter) {
                    v_r = (2.0 / 3.0) * vmax *
                          (gsl_rng_uniform_pos(r[tid]) - 0.5);
            ...
            if (fabs(vel_x) > maxvel_x)
              maxvel_x = fabs(vel_x);
            if (fabs(vel_y) > maxvel_y)
              maxvel_y = fabs(vel_y);
            if (fabs(vel_z) > maxvel_z)
              maxvel_z = fabs(vel_z);
        ...
        lambda_crit = sqrt(2.0) * pow(eps_m, 2.0) * gal->comp_mass_frac[j] *
                      sqrt(f_c_func(gal->comp_concentration[gal->index_halo])) /
                      (f_r_func(gal, j) * pow(f_v_func(gal, j), 2.0));

        gal->comp_Q_bar[j] =
            v_cmax / sqrt(G * gal->comp_mass[j] / gal->comp_scale_length[j]);
        J_sum += J_comp;
#pragma omp barrier
        printf("[J=%4.2le  Msol.%s.%s]", J_comp * unit_mass / solarmass,
               AllVars.UnitLengthName, AllVars.UnitVelocityName);
        if (gal->comp_type[j] != 0)
          printf("[    max vx=%4.2le vy=%4.2le vz=%4.2le  %s    ][ v_st_max = "
                 "%4.2le %s ]",
                 maxvel_x, maxvel_y, maxvel_z, AllVars.UnitVelocityName,
                 v_stream_max, AllVars.UnitVelocityName);
        if (gal->comp_epicycle[j] == 1) {
          printf(
              "\n/////\t\t---------------[  Q_min=%5.2lf  ][  Q_bar=%5.2lf  ]",
              gal->comp_Q_min[j], gal->comp_Q_bar[j]);
          if (gal->lambda > 0.)
            printf("[    lambda=%4.2le   lambda_crit=%4.2le    ]",
                   gal->lambda * gal->comp_angmom_frac[j] /
                       gal->comp_mass_frac[j],
                   lambda_crit);
        }
        if (gal->comp_thermal_eq[j] == 1)
          printf("[   thermal equilibirum   ]");
        if (gal->comp_type[j] == 0)
          printf("[ v_st_max = %4.2le %s ]", v_stream_max,
                 AllVars.UnitVelocityName);
        printf("\n");
        if (J_sum > 1.01 * gal->J200)
          printf("/////\t\t---------------[         Warning         ][         "
                 "      sum(J_component) > J200               ]\n");
        if (nrejected > 1e-3) {
          if (gal->comp_jeans_dim[j] == 1) {
            printf("/////\t\t---------------[         Warning         ][      "
                   "rejection = %4.1lf%% / failed rejection=%4.1lf%%     ]\n",
        ...
        if (gal->comp_theta_sph[j] != 0. || gal->comp_phi_sph[j] != 0.)
          rotate_all(gal, gal->comp_theta_sph[j], gal->comp_phi_sph[j], 1);
      }
    }
    if (warning1)
      printf("/////\t\t---------------[         Warning         ][           "
             "Streaming velocity > <v2_theta>           ]\n");
    warning1 = 0;
    if (warning2)
      printf("/////\t\t---------------[         Warning         ][ Gaseous "
             "halo unstable -> Lower streaming_fraction%d  ]\n",
             j + 1);
    warning2 = 0;
    ...
    if (AllVars.OutputSigma == 1) {
      if (gal->comp_npart[j] > 0) {
        ...
        write_galaxy_rz_quantities(
            gal, min(2.0 * gal->comp_cut[gal->selected_comp[0]], gal->maxrad),
            strcat(buffer, ext), interval);
      }
    }
    gal->pseudo[tid] = 0;
  }
  ...
  printf("/////\t\t[   J_tot=%4.2le  J200=%4.2le Msol.%s.%s  ]\n",
         J_sum * unit_mass / solarmass, gal->J200 * unit_mass / solarmass,
         AllVars.UnitLengthName, AllVars.UnitVelocityName);
  if (warning1)
    printf("/////\t\t[Warning] Potential derivative unstable -> Increase "
           "particule number\n");
...
int set_stream_velocity(stream *st) {
  ...
  double sigma_theta;
  double v2a_r, v2a_theta, v2_theta, va_theta, vel_x, vel_y, vel_z, stddev_vx,
      stddev_vy, stddev_vz;
  ...
  for (k = 0; k < AllVars.MaxCompNumber; k++) {
    if (st->comp_turb_sigma[k] > 0.) {
      ...
      printf("/////\t\t- Component %2d -> setting turbulence [sigma=%.2lf "
             "km/s][scale inj=%.2lf kpc][scale diss=%.3lf kpc][spectral "
             "index=%.2lf]\n",
             k + 1, st->comp_turb_sigma[k], st->comp_turb_scale_inj[k],
             st->comp_turb_scale_diss[k], st->comp_turb_nspec[k]);
      printf("/////\t\t                                     [grid scale=%.3lf "
             "kpc][seed=%ld]\n",
             st->dx_gauss, st->comp_turb_seed[k]);
      set_stream_random_field_grid(
          st, st->comp_turb_scale_inj[k], st->comp_turb_scale_diss[k],
          st->comp_turb_nspec[k], st->comp_turb_seed[k]);
      for (i = st->comp_start_part[k];
           i < st->comp_start_part[k] + st->comp_npart[k]; ++i) {
        st->vel_x[i] +=
            stream_gaussian_field_func(st, st->x[i], st->y[i], st->z[i]);
      }
      set_stream_random_field_grid(
          st, st->comp_turb_scale_inj[k], st->comp_turb_scale_diss[k],
          st->comp_turb_nspec[k], st->comp_turb_seed[k] + 1);
      for (i = st->comp_start_part[k];
           i < st->comp_start_part[k] + st->comp_npart[k]; ++i) {
        st->vel_y[i] +=
            stream_gaussian_field_func(st, st->x[i], st->y[i], st->z[i]);
      ...
      set_stream_random_field_grid(
          st, st->comp_turb_scale_inj[k], st->comp_turb_scale_diss[k],
          st->comp_turb_nspec[k], st->comp_turb_seed[k] + 2);
      for (i = st->comp_start_part[k];
           i < st->comp_start_part[k] + st->comp_npart[k]; ++i) {
        st->vel_z[i] +=
            stream_gaussian_field_func(st, st->x[i], st->y[i], st->z[i]);
      ...
      deallocate_stream_gaussian_grid(st);
      // Computing standard deviations
      stddev_vx = standard_dev(st->vel_x, st->comp_start_part[k],
                               st->comp_start_part[k] + st->comp_npart[k]);
      stddev_vy = standard_dev(st->vel_y, st->comp_start_part[k],
                               st->comp_start_part[k] + st->comp_npart[k]);
      stddev_vz = standard_dev(st->vel_z, st->comp_start_part[k],
                               st->comp_start_part[k] + st->comp_npart[k]);
      // Rescaling velocities to get the user specified dispersion
      for (i = st->comp_start_part[k];
           i < st->comp_start_part[k] + st->comp_npart[k]; ++i) {
    ...
    for (n = 0; n < gal->nlevel; n++) {
      for (i = 0; i < 2 * gal->ngrid[n][0]; i++) {
        for (j = 0; j < 2 * gal->ngrid[n][1]; j++) {
          ...
          if (n > gal->nlevel)
            free(gal->potential_ext[n][i][j]);
        }
        free(gal->potential[n][i]);
        if (n > gal->nlevel)
          free(gal->potential_ext[n][i]);
      }
      free(gal->potential[n]);
      if (n > gal->nlevel)
        free(gal->potential_ext[n]);
    }
    free(gal->potential);
    if (n > gal->nlevel)
      free(gal->potential_ext);
  }
  if (gal->midplane_dens_defined == 1) {
    for (i = 0; i < gal->ngrid_dens[1]; i++) {
...
int rotate_galaxy(galaxy *gal, int index) {
  ...
  for (i = AllVars.GalStart[index];
       i < AllVars.GalStart[index] + AllVars.GalNpart[index]; ++i) {
...
// This functions performs a rotation of all the particles contained in a single
// component Angles should be specified in degrees
...
int rotate_stream(galaxy *st, int index) {
  ...
  for (i = AllVars.StreamStart[index];
       i < AllVars.StreamStart[index] + AllVars.StreamNpart[index]; ++i) {
    // Rotation around Y axis
...
// This function intend to setup the global postion velocity vectors of a
// galaxy, in order to test different combination of ICs for galaxy collision
// simulations.
...
int copy_galaxy(galaxy *gal_1, galaxy *gal_2, int info) {
  ...
  for (j = 0; j < AllVars.MaxCompNumber; ++j) {
    if (gal_2->comp_npart[j] > 0) {
      ...
      for (i = gal_1->comp_start_part[j];
           i < gal_1->comp_start_part[j] + gal_1->comp_npart[j]; ++i) {
        gal_2->x[k] = gal_1->x[i];
        gal_2->y[k] = gal_1->y[i];
        gal_2->z[k] = gal_1->z[i];
...
int add_galaxy_to_system(galaxy *gal_1, galaxy *gal_2) {
  ...
  for (k = 0; k < AllVars.MaxCompNumber; k++) {
    if (gal_1->comp_npart[k] > 0) {
      ...
      gal_2->comp_start_part[j] =
          gal_2->ntot_part_pot + gal_1->comp_start_part[k];
      gal_2->comp_delete[j] = gal_1->comp_delete[k];
      if (l == 0) {
        gal_2->comp_start_part[j] = gal_2->ntot_part;
      } else {
        gal_2->comp_start_part[j] =
            gal_2->comp_start_part[j - 1] + gal_2->comp_npart[j - 1];
      }
      gal_2->comp_type[j] = gal_1->comp_type[k];
      j++;
      l++;
      if (j == AllVars.MaxCompNumber) {
        printf("[Error] No more free components for the system -> Increase "
               "MaxCompNumber\n");
        return -1;
      }
    }
  ...
  if (reallocate_variable_arrays(gal_2, gal_1->ntot_part + gal_2->ntot_part) !=
      0) {
  ...
  for (k = 0; k < AllVars.MaxCompNumber; k++) {
    if (gal_1->comp_npart[k] > 0) {
      for (i = gal_1->comp_start_part[k];
           i < gal_1->comp_start_part[k] + gal_1->comp_npart[k]; ++i) {
...
// Transforms a stream object into a galaxy structure and add it to the galaxy
// stack
int add_stream_to_system(stream *st_1, galaxy *gal_2) {
  ...
  for (k = 0; k < AllVars.MaxCompNumber; k++) {
    if (st_1->comp_npart[k] > 0) {
      ...
      gal_2->comp_start_part[j] =
          gal_2->ntot_part_pot + st_1->comp_start_part[k];
      gal_2->comp_type[j] = 0;
      j++;
    }
  }
  ...
  // Reallocate variable arrays
  if (reallocate_variable_arrays(gal_2,
                                 st_1->ntot_part + gal_2->ntot_part_pot) != 0) {
    fprintf(stderr, "Allocation of component arrays failed\n");
  }
...
// plane. The orbit is fully parametrizable going from hyperbolic orbits to
// elliptic orbits, including the special case of parabolic orbits.
void set_orbit_keplerian(int gal1, int gal2, double sep, double per, double e,
                         double phi, double theta, int center) {

  unsigned long int i;
  double nu1, nu2, x_1, y_1, z_1, x_2, y_2, z_2, vx_1, vy_1, vz_1, vx_2, vy_2,
      vz_2, mu, angmom;
  double radius1, radius2, a1, a2, l1, l2, k1, k2, v_ini, r_ini,
      specific_orbital_energy, kappa, l, theta_diago, degtorad;
  double xt_1, yt_1, zt_1, vxt_1, vyt_1, vzt_1, xt_2, yt_2, zt_2, vxt_2, vyt_2,
      vzt_2;
  double m1, m2;
  ...
  mu = (m1 * m2) / (m1 + m2);
  if (per > sep) {
    fprintf(stderr, "[Warning] The pericentral distance must be lower than the "
                    "initial distance");
    fprintf(stderr, "[Warning] Setting initial distance value equal to "
                    "pericentral distance value");
    sep = per;
  }
  // Mass ratios
  k1 = mu / m1;
  ...
  if (e == 1)
    ...
    a1 = k1 * per / fabs(1.0 - e);
  if (e == 1)
    a2 = k2 * per;
  else
    a2 = k2 * per / fabs(1.0 - e);
  // Semi-latus rectum
  if (e == 1)
    l1 = 2.0 * a1;
  else
  ...
  printf("/////\t\t- Center of galaxy 1           -> "
         "[x=%5.1lf,y=%5.1lf,z=%5.1lf] %s\n",
         x_1, y_1, z_1, AllVars.UnitLengthName);
  printf("/////\t\t- Center of galaxy 2           -> "
         "[x=%5.1lf,y=%5.1lf,z=%5.1lf] %s\n",
         x_2, y_2, z_2, AllVars.UnitLengthName);
  printf("/////\t\t- Velocity of galaxy 1         -> "
         "[vx=%6.1lf,vy=%6.1lf,vz=%6.1lf] %s\n",
         vx_1, vy_1, vz_1, AllVars.UnitVelocityName);
  printf("/////\t\t- Velocity of galaxy 2         -> "
         "[vx=%6.1lf,vy=%6.1lf,vz=%6.1lf] %s\n",
...
void set_orbit_circular(int gal1, int gal2, double sep, double vc, double phi,
                        double theta, int center) {
  unsigned long int i;
